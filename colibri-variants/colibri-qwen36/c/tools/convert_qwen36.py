...
def quantize_group(w: torch.Tensor, bits: int, gs: int) -> tuple[torch.Tensor, torch.Tensor]:
    """Symmetrically quantize a 2D [output, input] matrix in input-axis groups.

    The returned scales are flattened from row-major [output, ceil(input / gs)].
    """
    if w.ndim != 2:
        raise ValueError(f"group quantization requires a 2D tensor, got shape {tuple(w.shape)}")
    if gs <= 0:
        raise ValueError(f"group size must be positive, got {gs}")

    qmax = (1 << (bits - 1)) - 1
    w_f32 = w.float()
    outputs, inputs = w_f32.shape
    groups = math.ceil(inputs / gs)
    q = torch.empty((outputs, inputs), dtype=torch.int8, device=w_f32.device)
    scales = torch.empty((outputs, groups), dtype=torch.float32, device=w_f32.device)

    full_groups = inputs // gs
    if full_groups:
        full_inputs = full_groups * gs
        grouped = w_f32[:, :full_inputs].reshape(outputs, full_groups, gs)
        full_scales = grouped.abs().amax(dim=2).clamp(min=1e-12) / qmax
        scales[:, :full_groups] = full_scales
        q[:, :full_inputs] = (
            grouped / full_scales.unsqueeze(2)
        ).round().clamp(-qmax - 1, qmax).to(torch.int8).reshape(outputs, full_inputs)

    if full_groups < groups:
        tail = w_f32[:, full_groups * gs:]
        tail_scale = tail.abs().amax(dim=1).clamp(min=1e-12) / qmax
        scales[:, full_groups] = tail_scale
        q[:, full_groups * gs:] = (
            tail / tail_scale.unsqueeze(1)
        ).round().clamp(-qmax - 1, qmax).to(torch.int8)

    return q, scales.flatten().contiguous()
...
def make_merged(gate, up, down, ebits, gs=0):
    ...
    qs contains one f32 scale per output row when gs=0. When gs>0 it contains
    row-major [output, ceil(input / gs)] scales for gate, then up, then down.
    """
    def q(t):
        matrix = t.reshape(t.shape[0], -1)
        if gs > 0:
            return quantize_group(matrix, ebits, gs)
        return quantize_row(matrix, ebits)

    gq, gate_scales = q(gate)
    uq, up_scales = q(up)
    dq, down_scales = q(down)
    ...
    qs = torch.cat([gate_scales, up_scales, down_scales]).contiguous().float()
    return mw, qs
...
def _selftest():
    """Round-trip checks for true-int4 packing and group-scaled quantization."""
    ...
    # Confirm ebits>=5 still stores int8.
    g = torch.randn(8, 16) * 3
    u = torch.randn(8, 16) * 3
    d = torch.randn(16, 8) * 3
    mw8, _ = make_merged(g, u, d, 8)
    print(f"  ebits=8: dtype={mw8.dtype} (expected int8), bytes={mw8.numel()}")
    ok = ok and (mw8.dtype == torch.int8)

    # Exercise the gs64 container layout with a small group size suitable for
    # the synthetic matrices. Each projection's scales must be [O][ceil(I/gs)].
    group_size = 4
    _, gate_scales = quantize_group(g, 4, group_size)
    _, up_scales = quantize_group(u, 4, group_size)
    _, down_scales = quantize_group(d, 4, group_size)
    _, merged_scales = make_merged(g, u, d, 4, gs=group_size)
    expected_counts = (
        g.shape[0] * math.ceil(g.shape[1] / group_size),
        u.shape[0] * math.ceil(u.shape[1] / group_size),
        d.shape[0] * math.ceil(d.shape[1] / group_size),
    )
    actual_counts = (gate_scales.numel(), up_scales.numel(), down_scales.numel())
    segment_counts_ok = actual_counts == expected_counts
    merged_layout_ok = (
        merged_scales.numel() == sum(expected_counts)
        and torch.equal(
            merged_scales,
            torch.cat([gate_scales, up_scales, down_scales]).float(),
        )
    )
    print(f"  gs={group_size}: scale_counts={actual_counts} expected={expected_counts} "
          f"segments={segment_counts_ok} merged_layout={merged_layout_ok}")
    ok = ok and segment_counts_ok and merged_layout_ok
...
def main():
    ...
    ap.add_argument("--gs", type=int, default=0,
                    help="Expert input-axis quantization group size (0=row-wise, default 0)")
    ...
    if args.gs < 0:
        sys.exit(f"--gs must be >= 0 (got {args.gs})")
    ...
    # VL checkpoints nest the language-model dimensions under text_config. Publish
    # those fields at the root because the engine reads a flat config.json.
    text_cfg = cfg_full.get("text_config")
    mcfg = dict(text_cfg if isinstance(text_cfg, dict) else cfg_full)
    if "vocab_size" not in mcfg:
        if isinstance(text_cfg, dict) and "vocab_size" in text_cfg:
            mcfg["vocab_size"] = text_cfg["vocab_size"]
        elif "vocab_size" in cfg_full:
            mcfg["vocab_size"] = cfg_full["vocab_size"]
    for key in ("layer_types", "full_attention_interval"):
        if key in (text_cfg or {}):
            mcfg[key] = text_cfg[key]
    rope_parameters = mcfg.get("rope_parameters")
    if "rope_theta" not in mcfg and isinstance(rope_parameters, dict):
        if "rope_theta" in rope_parameters:
            mcfg["rope_theta"] = rope_parameters["rope_theta"]

    out = Path(args.out)
    out.mkdir(parents=True, exist_ok=True)
    import shutil
    shutil.copy2(cfg_path, out / "config.hf.json")
    (out / "config.json").write_text(json.dumps(mcfg, indent=2) + "\n", encoding="utf-8")
    print(f"config -> {out / 'config.json'} (model_type={cfg_full.get('model_type')})")

    # Bundle the tokenizer so the container can run text-prompt mode without
    # consulting the source checkpoint.
    tokenizer_names = ("tokenizer.json", "tokenizer_config.json", "special_tokens_map.json")
    for tokenizer_name in tokenizer_names:
        if args.repo:
            try:
                tokenizer_src = Path(hf_hub_download(args.repo, tokenizer_name, token=token))
            except Exception as exc:
                if tokenizer_name == "tokenizer.json":
                    sys.exit(f"required {tokenizer_name} missing from {args.repo}: {exc}")
                continue
        else:
            tokenizer_src = src_dir / tokenizer_name
            if not tokenizer_src.is_file():
                if tokenizer_name == "tokenizer.json":
                    sys.exit(f"required {tokenizer_name} missing in {src_dir}")
                continue
        tokenizer_dst = out / tokenizer_name
    ...
    for i in all_idx:
        ...
        for e in sorted(sep):
            ...
            if "gate_proj" in d and "up_proj" in d and "down_proj" in d:
                mw, qs = make_merged(
                    d["gate_proj"], d["up_proj"], d["down_proj"], args.ebits, gs=args.gs
                )
                tens[f"model.layers.{a}.mlp.experts.{e}.merged_weight"] = mw
                tens[f"model.layers.{a}.mlp.experts.{e}.qs"] = qs
            else:
    ...
    if args.upload_repo:
        ...
        upload_files = [
            out / "config.json", out / "config.hf.json", out / "qwen36_meta.json",
            out / "tokenizer.json", out / "tokenizer_config.json",
            out / "special_tokens_map.json", out / "README.md",
        ]
        for f in sorted(out.glob("*.safetensors")) + upload_files:
            if f.is_file():
                stream_api.upload_file(repo_id=args.upload_repo, repo_type="model",

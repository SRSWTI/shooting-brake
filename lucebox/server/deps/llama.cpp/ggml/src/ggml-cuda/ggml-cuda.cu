        ...
        for (int64_t i12 = 0; i12 < ne12; ++i12) { // tokens
            for (int64_t iex = 0; iex < n_expert_used; ++iex) {
                ...
                if (expert_to_use == i02) {
                    ...
                    // Multiple route slots may intentionally reference the same
                    // expert (for example, zero-weight hybrid-offload dummies).
                    // Preserve every slot so ids_from_sorted remains complete.
                }
            }
        }
    }

...
static void print_usage(const char * prog) {
    std::fprintf(stderr,
        ...
        "  --spark             Enable self-tuning hot/cold MoE expert placement\n"
        "  --spark-vram <GiB>  Target total GPU memory for Spark placement\n"
        "  --spark-slots <N>   Expert cache slots per layer (-1=auto)\n"
        "  --spark-ssd         Keep Spark's cold expert tier on SSD instead of RAM\n"
...
int main(int argc, char ** argv) {
    ...
    bool   spark_ssd = false;       // --spark-ssd: file-backed cold experts
    ...
    for (int i = 2; i < argc; i++) {
        if (std::strcmp(argv[i], "--draft") == 0 && i + 1 < argc) {
            ...
            spark_autotune = true;
        } else if (std::strcmp(argv[i], "--spark-ssd") == 0) {
            spark_autotune = true;
            spark_ssd = true;
    ...
    if (spark_autotune) {
        ...
        if (arch_has_expert_offload(arch)) {
            ...
            if (spark_ssd) ::setenv("DFLASH_SPARK_SSD", "1", 1);

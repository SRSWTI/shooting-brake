...
//
// Same semantics as K5 (gemm_rmsnorm_partial_cross_entropy) but eliminates the
// post-GEMM re-read of M×N logits by computing per-tile partial LSE in-epilogue:
//
...
struct GemmRmsNormPartialCrossEntropyFused {
  ...
  // ── LseTree: per-tile logsumexp → partial buffer ─────────────────────────

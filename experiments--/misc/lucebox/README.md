...
| `--spark-ssd` | off | Keep cold experts file-backed in the GGUF; service misses through bounded pinned staging and GPU cache slots. Implies `--spark`. |
| `DFLASH_SPARK=1` | off | Env equivalent of `--spark`. |
| `DFLASH_SPARK_VRAM_MB=N` | — | Env equivalent of `--spark-vram` (in MB). |
| `DFLASH_SPARK_SSD=1` | off | Env equivalent of `--spark-ssd`. |
| `DFLASH_SPARK_SSD_STAGING_SLOTS=N` | routed experts/token | Pinned staging slots for concurrent cold-expert reads; valid range is routed experts/token through 64. |

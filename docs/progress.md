...
| 8 — Piecewise CUDA graphs and overlap | **Phase 8a complete; async-overlap gate passed (2026-08-05)** | **8a (async overlap):** The SYCL `issue()` already submits the kernel asynchronously. Phase 8a splits the Python binding into `issue()` / `take()` and reorders `forward_modular`: issue B70 before CUDA routed-expert compute, take after — so the B70 kernel (~44µs) overlaps with CUDA (~100µs+). Pinned host buffers (`pin_memory=True, device="cpu"`) enable DMA D2H/H2D. `phase8/async_overlap_test.py` passes with three-way exact token parity (all-CUDA = async-B70 = sync-B70). `SHOOTING_BRAKE_B70_ASYNC=1` (default) enables overlap; `=0` falls back to synchronous Phase 7. **8b (piecewise CUDA graphs):** Deferred. Standard vLLM graph capture can't span the cross-device break; the NVFP4 model has good eager performance; overlap already hides the B70 stall behind CUDA compute. |
...
Work proceeds in the Phase 0–10 order from [`../plan.md`](../plan.md). Phases 0 through 8a are complete. The immediate deliverable is:

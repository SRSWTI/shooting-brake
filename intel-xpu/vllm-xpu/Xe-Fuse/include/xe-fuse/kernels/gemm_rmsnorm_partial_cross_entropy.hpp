...
// K5: gemm_rmsnorm_partial_cross_entropy — D[m,n] = rstd[m] * acc[m,n]
//
// RMSNorm scaling via per-row broadcast of rstd (same epilogue as K1).
// Post-GEMM (standalone kernels):
//   lse[m]         = log(sum_n exp(D[m,n]))
//   logits_tgt[m]  = D[m, targets[m]]
//
// kernel_5 from CODA (arxiv.org/abs/2605.19269): vocab-projection GEMM with
// RMSNorm normalization and cross-entropy statistics. See K9 for the unscaled
// variant (no rstd).
...
>
struct GemmRmsNormPartialCrossEntropy {
  ...
  // EVT: D[m,n] = rstd[m] * acc[m,n]  (identical to K1 / GemmRmsNorm)
  ...
  static typename EVT::Arguments make_evt_args(ElementScale const* rstd_ptr, int M) {
    ...
    return {accum_args, scale_args, mul_args};
  }

  // Post-GEMM: compute log-sum-exp per row from scaled logits
  ...
  // Post-GEMM: extract target logit per row from scaled logits

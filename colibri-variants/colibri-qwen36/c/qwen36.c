...
typedef struct {
    ...
    int eos_ids[8];     /* generation stops when argmax hits one of these */
    int n_eos;          /* number of valid eos_ids entries (0 = no stop) */
} Cfg;
...
#ifdef COLI_QWEN_CUDA_STATE
    QwenCudaState *cuda;
    int cuda_greedy, cuda_greedy_valid;
#endif
} Model;
...
#ifdef COLI_QWEN_CUDA_STATE
static QwenCudaWeight qcw(const float *w,int I,int O){
    QwenCudaWeight r={w,NULL,0,I,O,0};
    for(int i=0;i<g_qdw_n;i++) if(g_qdw[i].w==w&&g_qdw[i].I==I&&g_qdw[i].O==O){
        r.weights=g_qdw[i].q; r.scales=g_qdw[i].sc; r.fmt=1; break;
    }
    return r;
}
#endif
...
static void load_cfg(Cfg *c, const char *snap) {
    ...
    /* EOS ids: EOS_IDS env (comma-separated) overrides config.json. */
    c->n_eos = 0;
    { const char *ei = getenv("EOS_IDS");
      if (ei && *ei) {
          char eb[256]; strncpy(eb, ei, sizeof(eb)-1); eb[sizeof(eb)-1]=0;
          for (char *t=strtok(eb,","); t && c->n_eos<8; t=strtok(NULL,","))
              c->eos_ids[c->n_eos++] = atoi(t);
      } else {
          jval *ev = json_get(r,"eos_token_id");
          if (ev && ev->t==J_NUM) { c->eos_ids[0]=(int)ev->num; c->n_eos=1; }
          else if (ev && ev->t==J_ARR)
              for (int i=0;i<ev->len && c->n_eos<8;i++)
                  if (ev->kids[i]->t==J_NUM) c->eos_ids[c->n_eos++]=(int)ev->kids[i]->num;
      }
    }
    free(buf); free(arena);
...
static void model_init(Model *m, const char *snap, int cap, int bits) {
    ...
    load_cfg(&m->c, snap);
    load_meta(&m->c, snap);
    /* Shared quantized-matmul layout state is needed by both the direct CLI and
     * qwen36_serve; initializing it in main left server requests on per-row scales. */
...
static void load_expert_merged(Model *m, int layer, int eid, Slot *s) {
    ...
    if (tw->nbytes == want_w / 2) {
        ...
        /* Always keep the packed int4 bytes for the int4 GPU shader, regardless
         * of whether the Vulkan backend is currently initialised.  Gating this
         * on vg_use_int4() created a load/initialisation-order race: experts can
         * be loaded (and the GPU slot cache stamped "valid") before vg_init() has
         * run, leaving g4 empty and the GPU reading all-zero weights while the
         * CPU int8 fallback (s->g) stayed correct.  The unpacked int8 copy in s->g
...
/* COLIBRI_RESIDENT: after prefill, pin every expert this prompt actually routed to.
 * CPU LRU then never evicts them, so vg_expert_ensure_int4 keeps hitting its valid
 * slot in g_w -> the expert stays resident in GPU memory for the whole decode. */
/* COLIBRI_RESIDENT: after prefill (mode 1) and continuously through decode (mode 2),
 * pin every expert this prompt routed to. CPU LRU then never evicts them, so
 * vg_expert_ensure_int4 keeps hitting its valid slot in g_w -> expert stays resident
 * in GPU memory. `quiet` suppresses the log line when nothing new was pinned
...
#ifdef COLI_QWEN_CUDA_STATE
static int qwen_cuda_cpu_routes_cb(void *opaque,int layer,const int *eids,
        const float *weights,int K,uint32_t mask,const float *x,float *partial){
    Model *m=(Model*)opaque; Cfg *c=&m->c;
    float *g=falloc(c->inter),*u=falloc(c->inter),*h=falloc(c->hidden);
    for(int k=0;k<K;k++) if(mask&(1u<<k)){
        Slot *e; expert_get(m,layer,eids[k],&e); slot_ensure_int8(m,e);
        matmul_qe(g,x,e->g,e->gs,c->hidden,c->inter);
        matmul_qe(u,x,e->u,e->us,c->hidden,c->inter);
        for(int j=0;j<c->inter;j++){float v=g[j];g[j]=v/(1.f+expf(-v))*u[j];}
        matmul_qe(h,g,e->d,e->ds,c->inter,c->hidden);
        for(int d=0;d<c->hidden;d++) partial[d]+=weights[k]*h[d];
    }
    free(g);free(u);free(h);return 1;
}
static void qwen_cuda_route_commit_cb(void *opaque,int layer,const int *eids,
        const float *weights,int K){
    (void)weights;
    Model *m=(Model*)opaque; Cfg *c=&m->c;
    for(int k=0;k<K;k++) if(eids[k]>=0){
        if(m->resident_collecting) m->seen[(int64_t)layer*c->n_experts+eids[k]]=1;
        if(!m->hot_pinned&&m->freq) m->freq[(int64_t)layer*c->n_experts+eids[k]]++;
        Slot *e; expert_get(m,layer,eids[k],&e);
        if(e->g4) qt_note(layer,eids[k],e->g4,e->u4,e->d4,e->gs,e->us,e->ds);
    }
}
#endif

/* Shared CLI/server startup: quantize once, upload every dense/state allocation
 * before freeing F32 originals, then let qt_init budget experts from remaining
 * free VRAM. */
static void prepare_dense_and_cuda(Model *m){
    double tq=now_s(); Cfg *c=&m->c; int D=c->hidden;
    int qo=c->q_heads*c->q_head_dim,kvo=c->kv_heads*c->k_head_dim;
    if(dense_i8_on()){
        qdw_register(m->embed,D,c->vocab);
        for(int i=0;i<c->n_layers;i++){
            Layer *l=&m->L[i];
            qdw_register(l->q,D,qo);qdw_register(l->k,D,kvo);qdw_register(l->v,D,kvo);
            qdw_register(l->o,c->o_in,D);qdw_register(l->gate,D,c->n_experts);
            qdw_register(l->sh_g,D,c->shared_inter);qdw_register(l->sh_u,D,c->shared_inter);
            qdw_register(l->sh_d,c->shared_inter,D);
            qdw_register(l->dn_qkv,D,c->dn_conv_dim);
            qdw_register(l->dn_z,D,c->dn_vheads*c->dn_vdim);
            qdw_register(l->dn_out,c->dn_vheads*c->dn_vdim,D);
        }
        qdw_register(m->lm_head,D,c->vocab);
    }
#ifdef COLI_QWEN_CUDA_STATE
    QwenCudaLayerWeights *lw=calloc((size_t)c->n_layers,sizeof(*lw));
    if(lw){
        for(int i=0;i<c->n_layers;i++){
            Layer *l=&m->L[i]; QwenCudaLayerWeights *d=&lw[i];
            d->is_attention=c->is_attn[i];
            d->q=qcw(l->q,D,qo);d->k=qcw(l->k,D,kvo);d->v=qcw(l->v,D,kvo);
            d->o=qcw(l->o,c->o_in,D);d->router=qcw(l->gate,D,c->n_experts);
            d->shared_gate=qcw(l->sh_g,D,c->shared_inter);
            d->shared_up=qcw(l->sh_u,D,c->shared_inter);
            d->shared_down=qcw(l->sh_d,c->shared_inter,D);
            d->dn_qkv=qcw(l->dn_qkv,D,c->dn_conv_dim);
            d->dn_z=qcw(l->dn_z,D,c->dn_vheads*c->dn_vdim);
            d->dn_b=qcw(l->dn_b,D,c->dn_vheads);
            d->dn_a=qcw(l->dn_a,D,c->dn_vheads);
            d->dn_out=qcw(l->dn_out,c->dn_vheads*c->dn_vdim,D);
            d->input_norm=l->in_ln;d->post_norm=l->post_ln;d->q_norm=l->qn;d->k_norm=l->kn;
            d->router_bias=l->gate_bias;d->shared_gate_weight=l->sh_gate;
            d->dn_conv=l->dn_conv;d->dn_dt_bias=l->dn_dtbias;d->dn_a_log=l->dn_alog;d->dn_norm=l->dn_norm;
        }
        QwenCudaConfig cc={D,c->n_layers,c->vocab,c->q_heads,c->kv_heads,c->head_dim,
            c->q_head_dim,c->k_head_dim,c->rotary_dim,c->n_experts,c->topk,c->n_group,
            c->topk_group,c->shared_inter,c->dn_vheads,c->dn_kheads,c->dn_kdim,c->dn_vdim,
            c->dn_convk,c->dn_conv_dim,c->eps,c->theta};
        QwenCudaWeight ew=qcw(m->embed,D,c->vocab),hw=qcw(m->lm_head,D,c->vocab);
        int kv=getenv("CUDA_KV_TOKENS")?atoi(getenv("CUDA_KV_TOKENS")):8192;
        if(kv<1)kv=8192;
        m->cuda=qwen_cuda_create(&cc,lw,&ew,&hw,m->final_norm,kv,
                                 qwen_cuda_cpu_routes_cb,qwen_cuda_route_commit_cb,m);
        free(lw);
    }
#endif
    double freed=0;
    if(dense_i8_on()&&!getenv("COLI_KEEP_F32")){
        for(int i=0;i<g_qdw_n;i++) if(g_qdw[i].w!=m->embed){
            freed+=(double)g_qdw[i].I*g_qdw[i].O*sizeof(float);
            free((void*)g_qdw[i].w);
        }
    }
    if(dense_i8_on()) fprintf(stderr,"[dense-i8] %d matrices quantized in %.1f s, %.1f GB f32 freed\n",
...
enum { GPU_BACKEND_CPU = 0, GPU_BACKEND_VULKAN = 1 };
static int g_gpu_backend = GPU_BACKEND_CPU;   /* chosen backend (set by gpu_probe) */
static int g_gpu_count   = 0;                 /* # compute-capable devices */
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out) {
    ...
    for (int s = 0; s < S; s++) {
        ...
        if (qt_ready()) {
            ...
            uint32_t qmask = qt_issue(layer, idx, val, K, xs);
            ...
            uint32_t fail = qt_take(qmask, val, K, out + (int64_t)s*D);
            /* fix #5: recompute GPU experts whose results couldn't be collected */
            for (int kk = 0; kk < K; kk++) {
                if (!(fail & (1u<<kk))) continue;
                Slot *e; expert_get(m, layer, idx[kk], &e);
                slot_ensure_int8(m, e);
                matmul_qe(g, xs, e->g, e->gs, D, I);
                matmul_qe(u, xs, e->u, e->us, D, I);
            ...
            for (int kk = 0; kk < K; kk++) {
                Slot *e; expert_get(m, layer, idx[kk], &e);
                int li = (int)(e - m->cache[layer].slots);   /* mirror CPU LRU slot index */
                if (e->g4) {
                    /* int4 packed weights -> robust Shader-only GPU path (no int8/dot-product) */
                    vg_expert_ensure_int4(layer, li, idx[kk], e->g4, e->u4, e->d4, e->gs, e->us, e->ds);
                } else {
                    vg_expert_ensure(layer, li, idx[kk], e->g, e->u, e->d, e->gs, e->us, e->ds);
                }
                handles[kk] = layer * m->cache[layer].cap + li;
            }
            vg_moe_run(layer, K, handles, val, xs, out + (int64_t)s*D);
            if (getenv("MOE_DBG") && layer == 0 && s == 0) {
                for (int dk = 0; dk < K; dk++) {
                    Slot *de; expert_get(m, layer, idx[dk], &de);
                    fprintf(stderr,"[MOE_DBG] kk%d eid=%d g4=%s g0=%d gs0=%.4f u4=%s u0=%d us0=%.4f d4=%s d0=%d ds0=%.4f\n",
                        dk, idx[dk],
                        de->g4?"Y":"N", (int)de->g[0], de->gs[0],
                        de->u4?"Y":"N", (int)de->u[0], de->us[0],
                        de->d4?"Y":"N", (int)de->d[0], de->ds[0]);
                }
                float *oref = falloc((int64_t)D);
                float *gg = falloc((int64_t)I), *uu = falloc((int64_t)I), *hh2 = falloc((int64_t)D);
                for (int kk = 0; kk < K; kk++) {
                    Slot *e; expert_get(m, layer, idx[kk], &e);
                    matmul_q(gg, xs, e->g, e->gs, D, I);
                    matmul_q(uu, xs, e->u, e->us, D, I);
                    for (int i = 0; i < I; i++) { float gv = gg[i]; gg[i] = (gv / (1.f + expf(-gv))) * uu[i]; }
                    matmul_q(hh2, gg, e->d, e->ds, I, D);
                    for (int d = 0; d < D; d++) oref[d] += val[kk] * hh2[d];
                }
                double dot=0,n1=0,n2=0; const float *oa = out + (int64_t)s*D;
                for (int d=0;d<D;d++){ dot+=oref[d]*oa[d]; n1+=oref[d]*oref[d]; n2+=oa[d]*oa[d]; }
                double cos = (n1>0&&n2>0)? dot/sqrt(n1*n2) : 0;
                double gnorm=0, cnorm=0; for(int d=0;d<D;d++){gnorm+=oa[d]*oa[d]; cnorm+=oref[d]*oref[d];}
                fprintf(stderr,"[MOE_DBG] L0 T0 GPU vs CPU MoE cos=%.5f |GPU|=%g |CPU|=%g\n", cos, sqrt(gnorm), sqrt(cnorm));
                free(oref); free(gg); free(uu); free(hh2);
            }
        } else {
            for (int kk = 0; kk < K; kk++) {
                Slot *e; expert_get(m, layer, idx[kk], &e);
...
static float *step_cpu(Model *m, const int *ids, int S, int pos_base) {
...
#ifdef COLI_QWEN_CUDA_STATE
    m->cuda_greedy_valid=0;
    int debug_cpu=getenv("DUMP_LAYERS")||getenv("DN_DBG")||getenv("MOE_DBG")||g_pilot;
    if(m->cuda&&qwen_cuda_request_enabled(m->cuda)&&qt_ready()&&!debug_cpu){
        if(m->resident_mode&&m->first_step) m->resident_collecting=1;
        float *logit=falloc(m->c.vocab);
        int greedy=0;
        if(qwen_cuda_step(m->cuda,ids,S,pos_base,logit,&greedy)){
            m->cuda_greedy=greedy;m->cuda_greedy_valid=1;
            m->kv_len=pos_base+S;m->token_count+=S;m->freq_token_count+=S;
            if(!m->hot_pinned&&m->hot_n>0&&m->freq_token_count>=m->warmup_tokens)
                pin_hot_experts(m);
            if(tm_on()){tm_add(S,5,0);if(S==1)g_tm_dec_tokens++;else g_tm_pre_tokens+=S;}
            if(m->resident_collecting){
                int prefill_end=m->first_step;
                apply_resident(m,prefill_end?0:1);
                if(prefill_end){m->first_step=0;if(m->resident_mode<2)m->resident_collecting=0;}
            }
            return logit;
        }
        free(logit);
        /* qwen_cuda_step restored its device snapshot. Mirror that committed
         * boundary before retrying the complete step through the CPU oracle. */
        if(!qwen_cuda_export_state(m->cuda,m->K,m->V,m->DN_rec,m->DN_conv,
                                   m->kv_len,m->max_t)){
            fprintf(stderr,"[qwen-cuda] rollback mirror failed; refusing partial-state fallback\n");
            exit(1);
        }
        qwen_cuda_disable_request(m->cuda);
        fprintf(stderr,"[qwen-cuda] whole-step failure at pos %d; request moved to CPU oracle\n",
                pos_base);
    }
#endif
    return step_cpu(m,ids,S,pos_base);
}
...
static void reset_cuda_request(Model *m){
#ifdef COLI_QWEN_CUDA_STATE
    m->cuda_greedy_valid=0;
    if(m->cuda&&(!qwen_cuda_ensure_kv(m->cuda,m->max_t)||!qwen_cuda_reset(m->cuda))){
        qwen_cuda_disable_request(m->cuda);
        fprintf(stderr,"[qwen-cuda] state reset/KV growth failed; request uses CPU oracle\n");
    }
#else
    (void)m;
#endif
...
static int generate(Model *m, const int *prompt, int np, int n_new, int *out) {
    Cfg *c = &m->c;
    const char *ignore_eos_env = getenv("IGNORE_EOS");
    int ignore_eos = ignore_eos_env && atoi(ignore_eos_env) != 0;
    m->max_t = np + n_new;
    reset_recurrent(m);
    ensure_kv(m);
    reset_cuda_request(m);
    m->kv_len = 0;
    ...
    for (int s = 0; s < n_new; s++) {
        int best = 0;
#ifdef COLI_QWEN_CUDA_STATE
        if(m->cuda_greedy_valid) best=m->cuda_greedy;
        else
#endif
        { float bv=logit[0]; for(int i=1;i<c->vocab;i++) if(logit[i]>bv){bv=logit[i];best=i;} }
        if (s == 0 && g_ttft < 0) g_ttft = now_s() - g_gen_t0;   /* record TTFT */
        if (g_stream) { stream_token(best); fflush(stdout); }
        /* EOS: stop generation when argmax hits a configured stop token */
        int is_eos = 0;
        for (int e = 0; e < c->n_eos; e++) if (best == c->eos_ids[e]) { is_eos = 1; break; }
        if (s == n_new - 1 || (is_eos && !ignore_eos)) {
            if (getenv("DUMP")) {
...
/* ===================== optional GPU backend probe =====================
 * Phase-1 of GPU support: at startup, transparently detect whether a
 * Vulkan compute-capable GPU (integrated or discrete) exists. If so the
 * engine will later use it; if not (no loader / no device) it silently
 * falls back to the CPU path. The user never configures anything.
 *
 * This probe does NOT link Vulkan at compile time. It dynamically loads
 * the platform Vulkan loader (vulkan-1.dll / libvulkan.so) via
 * LoadLibrary/dlopen and resolves the few symbols needed to enumerate
 * devices, so the default binary stays zero-dependency. Actual compute
 * (matmul_q on GPU) is a later phase; this step only reports availability. */
/* (g_gpu_backend / g_gpu_count / GPU_BACKEND_* are declared above moe().) */
...
typedef struct { uint32_t sType; const void *pNext; const char *pApplicationName; uint32_t applicationVersion; const char *pEngineName; uint32_t engineVersion; uint32_t apiVersion; } VkAppInfoMini;
typedef struct { uint32_t sType; const void *pNext; const void *pApplicationInfo; uint32_t enabledLayerCount; const char* const *ppEnabledLayerNames; uint32_t enabledExtensionCount; const char* const *ppEnabledExtensionNames; } VkInstCIMini;
typedef int  (*PFN_vkCreateInstance)(const VkInstCIMini*, const void*, VkInstance*);
...
static int gpu_probe(void) {
    ...
    /* 0) optional hard disable for debugging (COLIBRI_GPU=0 -> CPU only) */
    {
        const char *ge = getenv("COLIBRI_GPU");
        if (ge && *ge == '0') { fprintf(stderr, "[gpu] COLIBRI_GPU=0 -> CPU backend forced\n"); return 0; }
    }

    /* 1) locate the Vulkan loader dynamically (no compile-time link) */
#if defined(_WIN32)
    HMODULE lib = LoadLibraryA("vulkan-1.dll");
    if (!lib) { fprintf(stderr, "[gpu] no vulkan-1.dll present -> CPU backend\n"); return 0; }
    #define VK_SYM(n) ((void*)GetProcAddress(lib, n))
#else
    void *lib = dlopen("libvulkan.so", RTLD_NOW);
    if (!lib) lib = dlopen("libvulkan.so.1", RTLD_NOW);
    if (!lib) { fprintf(stderr, "[gpu] no libvulkan present -> CPU backend\n"); return 0; }
    #define VK_SYM(n) dlsym(lib, n)
#endif
    PFN_vkCreateInstance  f_create = (PFN_vkCreateInstance)VK_SYM("vkCreateInstance");
    PFN_vkEnumeratePhysicalDevices f_enum = (PFN_vkEnumeratePhysicalDevices)VK_SYM("vkEnumeratePhysicalDevices");
    PFN_vkGetPhysicalDeviceQueueFamilyProperties f_qf = (PFN_vkGetPhysicalDeviceQueueFamilyProperties)VK_SYM("vkGetPhysicalDeviceQueueFamilyProperties");
    PFN_vkDestroyInstance f_destroy = (PFN_vkDestroyInstance)VK_SYM("vkDestroyInstance");
    #undef VK_SYM
    if (!f_create || !f_enum || !f_qf || !f_destroy) { fprintf(stderr, "[gpu] vulkan loader missing required symbols -> CPU backend\n"); return 0; }

    /* 2) create a bare instance (no layers/extensions needed for probing) */
    VkAppInfoMini ai; memset(&ai,0,sizeof ai); ai.sType = VK_ST_APPLICATION_INFO; ai.apiVersion = (1u<<22);
    VkInstCIMini ci; memset(&ci,0,sizeof ci); ci.sType = VK_ST_INSTANCE_CREATE_INFO; ci.pApplicationInfo = &ai;
    VkInstance inst = NULL;
    if (f_create(&ci, NULL, &inst) != VK_RESULT_SUCCESS || !inst) { fprintf(stderr, "[gpu] vkCreateInstance failed -> CPU backend\n"); return 0; }

    /* 3) enumerate physical devices */
    uint32_t ndev = 0;
    f_enum(inst, &ndev, NULL);
    if (ndev == 0) { fprintf(stderr, "[gpu] vulkan instance ok but 0 devices -> CPU backend\n"); f_destroy(inst, NULL); return 0; }
    VkPhysicalDevice *devs = (VkPhysicalDevice*)calloc(ndev, sizeof(VkPhysicalDevice));
    f_enum(inst, &ndev, devs);

    /* 4) count devices exposing a compute-capable queue family */
    int compute_devs = 0;
    for (uint32_t i = 0; i < ndev; i++) {
        uint32_t nf = 0;
        f_qf(devs[i], &nf, NULL);
        if (nf == 0) continue;
        char *qf = (char*)calloc(nf, VK_QF_STRIDE);
        f_qf(devs[i], &nf, qf);
        for (uint32_t j = 0; j < nf; j++) {
            uint32_t flags = *(uint32_t*)(qf + (size_t)j*VK_QF_STRIDE + VK_QF_OFF_FLAGS);
            if (flags & VK_QUEUE_COMPUTE_BIT) { compute_devs++; break; }
        }
        free(qf);
    }
    free(devs);
    ...
    if (compute_devs > 0) {
        g_gpu_backend = GPU_BACKEND_VULKAN;
        g_gpu_count = compute_devs;
        fprintf(stderr, "[gpu] detected %d Vulkan compute device(s) -> GPU backend ENABLED\n", compute_devs);
    } else {
        fprintf(stderr, "[gpu] %u device(s) found but none compute-capable -> CPU backend\n", ndev);
    }
...
int main(int argc, char **argv) {
    ...
    const char *cuda_mode = getenv("COLI_CUDA");
    if (!(cuda_mode && *cuda_mode == '1'))
        gpu_probe();   /* Vulkan fallback is probed only when CUDA is not selected. */
    ...
    if (is_ref) {
        ...
        if (getenv("IGNORE_EOS") && atoi(getenv("IGNORE_EOS")) != 0)
            fprintf(stderr, "[enc] EOS stopping disabled for fixed-length trace\n");
    ...
    if (g_gpu_backend == GPU_BACKEND_VULKAN) {
        vg_cfg vc;
        vc.n_layers = m.c.n_layers;
        vc.hidden   = m.c.hidden;
        vc.inter    = m.c.inter;
        vc.cap      = cap;
        vc.topk     = m.c.topk;
        vc.weight_bits = container_is_int4(&m) ? 4 : 8;  /* by on-disk size, not meta.ebits */
        if (vg_init(&vc) == 0) {
            fprintf(stderr, "[gpu] MoE expert GEMVs -> Vulkan compute device\n");
            atexit(vg_shutdown);
        } else {
            fprintf(stderr, "[gpu] vg_init failed -> keeping CPU MoE path\n");
            g_gpu_backend = GPU_BACKEND_CPU;
        }
    }

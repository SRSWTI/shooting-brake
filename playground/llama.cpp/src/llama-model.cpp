...
static buft_list_t make_cpu_buft_list(const std::vector<llama_device> & devices, bool use_extra_bufts, bool no_host) {
    ...
    // GGML_EXPERT_SHARD=1: pinned host buffers (cudaMallocHost) are faulted and
    // GUP-pinned by the driver at allocation, before any NUMA policy can apply -
    // mbind relabels them but the pages stay physically interleaved and unmovable,
    // silently defeating expert-home placement. Shard mode therefore forces plain
    // CPU buffers for CPU-kept weights (cost: pageable instead of pinned transfers
    // when batch prefill streams expert weights to the GPU).
    const char * shard_env = getenv("GGML_EXPERT_SHARD");
    const bool expert_shard = shard_env && atoi(shard_env) != 0;
    if (!no_host && !expert_shard) {
    ...
    // expert-shard mode also skips repack buffer types: repacked layouts are opaque
    // to the fused MoE kernel's raw-quant reads (observed: heap corruption + broken
    // fusion pattern matching when exps tensors land in a repack buffer)
    if (use_extra_bufts && !expert_shard) {

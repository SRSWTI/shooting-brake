...
Keep the cold tier in the GGUF instead of allocating compact CPU expert
tensors with `--spark-ssd`. Cold misses are read through a bounded pinned-host
staging pool, uploaded into the same GPU cache slots, synchronized, and only
then published to the current token's routing map:

```bash
dflash_server models/laguna.gguf --spark --spark-ssd --spark-vram 28 --spark-slots 16
```

`--spark-ssd` implies `--spark`. It requires at least one cache slot per routed
expert (`--spark-slots >= n_expert_used`); the server rejects smaller values.
`DFLASH_SPARK_SSD=1` is the environment equivalent.
`DFLASH_SPARK_SSD_STAGING_SLOTS=N` sizes the pinned read pool from
`n_expert_used` through 64 slots. Cumulative cache/read/upload telemetry is
printed as `[spark-ssd] ...` during clean shutdown. This path bounds host
allocation; it does not promise that every read bypasses the operating-system
page cache.
...
| `DFLASH_SPARK_SSD 1` | Keep cold expert weights file-backed in the GGUF and fill bounded GPU cache slots through pinned staging (set by `--spark-ssd`). |
| `DFLASH_SPARK_SSD_STAGING_SLOTS N` | Pinned host read slots for SSD misses; defaults to `n_expert_used`, valid range `n_expert_used..64`. |

...
bool init_pipelined_decode_state(
    ...
    // RAM-backed nonlocal experts use exact CPU compute. In SSD mode every
    // selected miss is made GPU-resident before graph dispatch.
    out.expert_compute = !hybrid.has_ssd_store() &&
...
bool pipelined_decode_one_token(
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (!is_attn
            ...
            // The SSD tier fills every current-token miss before remapping.
            // The RAM cold tier keeps the existing opportunistic GPU cache.
            if (hybrid.has_ssd_store()) {
                std::string ssd_error;
                if (!dflash::common::moe_hybrid_ssd_cache_selected(
                        hybrid, il, global_ids, n_expert_used, backend, &ssd_error)) {
                    std::fprintf(stderr, "[qwen35moe-ssd] layer %d: %s\n",
                                 il, ssd_error.c_str());
                    return false;
                }
            } else if (storage.cache_slots > 0) {
        ...
        if (hybrid.has_ssd_store()) {
            std::string ssd_error;
            if (!dflash::common::moe_hybrid_ssd_cache_selected(
                    hybrid, il, state.routing_ids_buf.data(), n_expert_used,
                    backend, &ssd_error)) {
                std::fprintf(stderr, "[qwen35moe-ssd] layer %d: %s\n",
                             il, ssd_error.c_str());
                return false;
        ...
        if (rffn.valid()) {
            ...
            int n_cold = 0;
            int layer_cold_hits = 0;
            // RAM-backed Spark retains its asynchronous cache fill. SSD misses
            // were completed and published before entering this branch.
            if (!hybrid.has_ssd_store() && storage.cache_slots > 0) {
                for (int i = 0; i < n_expert_used; ++i) {
                    dflash::common::moe_hybrid_cache_swap_in(
                        storage, state.routing_ids_buf[(size_t)i], backend);
                }
            }
            for (int i = 0; i < n_expert_used; ++i) {

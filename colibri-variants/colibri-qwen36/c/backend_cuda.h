...
/* ---- Qwen3.6 state-owner primitives --------------------------------------
 * These entry points operate only on device pointers, except for the explicitly
 * documented tiny metadata/result transfers. They are additive: the generic
 * Colibri/MLA pipeline above keeps its existing semantics. */
COLI_CUDA_DLLEXPORT int coli_cuda_pipe_zero(int device,void *dst_dev,size_t bytes);
COLI_CUDA_DLLEXPORT int coli_cuda_pipe_copy(int device,void *dst_dev,
                                            const void *src_dev,size_t bytes);
COLI_CUDA_DLLEXPORT int coli_cuda_pipe_embedding(ColiCudaTensor *embedding,
                                                 float *out_dev,
                                                 const int *ids_host,int S);
/* Qwen3.5/3.6 RMSNorm uses (1 + weight), unlike the generic pipe RMSNorm. */
COLI_CUDA_DLLEXPORT int coli_cuda_qwen_rmsnorm(int device,float *y_dev,
                                               const float *x_dev,
                                               const float *weight_dev,
                                               int S,int D,float eps);
/* q_proj is [S,q_heads,q_head_dim] (query followed by output gate);
 * k/v_proj are [S,kv_heads,head_dim]. K/V are head-major persistent caches. */
COLI_CUDA_DLLEXPORT int coli_cuda_qwen_gqa(int device,float *attn_out_dev,
        float *q_proj_dev,float *k_proj_dev,const float *v_proj_dev,
        const float *q_norm_dev,const float *k_norm_dev,
        float *k_cache_dev,float *v_cache_dev,
        int S,int pos_base,int kv_capacity,int q_heads,int kv_heads,
        int head_dim,int q_head_dim,int k_head_dim,int rotary_dim,
        float eps,float theta);
/* Sequential causal convolution followed by the gated delta-rule recurrence.
 * conv_state and recurrent_state are authoritative persistent float32 arrays. */
COLI_CUDA_DLLEXPORT int coli_cuda_qwen_deltanet(int device,float *outv_dev,
        const float *qkv_dev,const float *z_dev,const float *b_dev,const float *a_dev,
        const float *conv_weight_dev,const float *dt_bias_dev,const float *a_log_dev,
        const float *norm_weight_dev,float *conv_state_dev,float *recurrent_state_dev,
        int S,int value_heads,int key_heads,int key_dim,int value_dim,
        int conv_kernel,int conv_dim,float eps);
/* Computes router projection, biased softmax, group-limited top-k and
 * unconditional top-k renormalization on device. Selected metadata is also
 * copied to the supplied host arrays for tier orchestration. */
COLI_CUDA_DLLEXPORT int coli_cuda_qwen_router(ColiCudaTensor *gate,
        const float *bias_dev,const float *x_dev,int S,int K,
        int n_group,int topk_group,int *eids_dev,float *weights_dev,
        int *eids_host,float *weights_host,float *logits_host);
COLI_CUDA_DLLEXPORT int coli_cuda_qwen_shared_gate_add(int device,float *out_dev,
        const float *shared_dev,const float *x_dev,const float *gate_weight_dev,
        int S,int D);
COLI_CUDA_DLLEXPORT int coli_cuda_pipe_argmax(int device,const float *logits_dev,
                                              int n,int *index_host,
                                              float *value_host);
/* Preallocates all resident grouped-expert scratch before warmstart. */
COLI_CUDA_DLLEXPORT int coli_cuda_expert_group_resident_reserve(int device,int D,int I);
#ifdef __cplusplus
}
#endif

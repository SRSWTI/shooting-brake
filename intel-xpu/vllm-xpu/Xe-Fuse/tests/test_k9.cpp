...
// xe-fuse test: K9 — gemm_partial_cross_entropy
// D[m,n] = acc[m,n]  (bare GEMM → logits)
// Post-GEMM:
//   lse[m]        = log(sum_n exp(D[m,n]))
//   logits_tgt[m] = D[m, targets[m]]

#include "xe-fuse/kernels/gemm_partial_cross_entropy.hpp"
...
using K9 = xe_fuse::GemmPartialCrossEntropy<>;
using GemmOp = K9::Gemm;
...
int main(int argc, const char** argv) {
  ...
  auto stride_C = cutlass::make_cute_packed_stride(K9::StrideC{}, make_shape(M, N, L));
  auto stride_D = cutlass::make_cute_packed_stride(K9::StrideD{}, make_shape(M, N, L));

  cutlass::DeviceAllocation<K9::ElementA> block_A(static_cast<size_t>(M) * K * L);
  cutlass::DeviceAllocation<K9::ElementB> block_B(static_cast<size_t>(K) * N * L);
  cutlass::DeviceAllocation<K9::ElementD> block_D(static_cast<size_t>(M) * N * L);
  cutlass::DeviceAllocation<K9::ElementD> block_ref_D(static_cast<size_t>(M) * N * L);
  ...
  auto evt_args = K9::make_evt_args();
  ...
  K9::launch_lse(q, block_D.get(), block_lse.get(), M, N, L);
  K9::launch_select_logits(q, block_D.get(), block_targets.get(),
  ...
  if (opts.verify) {
    ...
    cutlass::reference::device::GemmComplex(
      {M, N, K}, K9::ElementAcc(1), ref_A, cutlass::ComplexTransform::kNone,
      ref_B, cutlass::ComplexTransform::kNone, K9::ElementAcc(0),
      ref_C, ref_D, K9::ElementAcc(0), L, M * K, K * N, M * N, M * N);
    compat::wait();

    // Reference LSE and SelectLogits from ref_D
    ...
    bool d_passed = cutlass::reference::device::BlockCompareRelativelyEqual(
      ...
      static_cast<K9::ElementD>(0.05f), static_cast<K9::ElementD>(0.05f));
  ...
  if (opts.iterations > 0) {
    ...
    printf("xe-fuse K9 (GEMM+LSE+SelectLogits): [%4.3f]TFlop/s  (%6.4f)ms\n",

                   ...
                   config.disk_cache_cold_max_tokens}, backend)
{
    ...
    disk_cache_.init();
    status_html_path_ = resolve_share_asset("status.html");
    chat_html_path_ = resolve_share_asset("chat.html");
}

// Resolve a file under the runtime share directory.
std::string HttpServer::resolve_share_asset(const std::string & filename) {
...
#endif
    if (!exe_dir.empty()) {
            // 2a. <exe_dir>/share/<asset>  (build directory layout)
            {
                std::string path = exe_dir + "/share/" + filename;
                struct stat st;
                if (::stat(path.c_str(), &st) == 0) return path;
            }
            // 2b. <exe_dir>/../share/<asset>  (installed prefix layout)
            {
                std::string path = exe_dir + "/../share/" + filename;
                struct stat st;
                if (::stat(path.c_str(), &st) == 0) return path;
    ...
    // 3. ./share/<asset> (development)
    {
        struct stat st;
        std::string path = "share/" + filename;
        if (::stat(path.c_str(), &st) == 0) return path;
    }
    return {};
}
...
void HttpServer::handle_client(int fd) {
    ...
    // Bundled web interfaces.
    if (hr.method == "GET" &&
        (hr.path == "/status" || hr.path == "/chat")) {
        const bool is_chat = hr.path == "/chat";
        const std::string & asset_path =
            is_chat ? chat_html_path_ : status_html_path_;
        const char * asset_name = is_chat ? "chat.html" : "status.html";
        if (asset_path.empty()) {
            send_error(fd, 404, std::string(asset_name) +
                " not found. Set DFLASH_SHARE_DIR or place it in share/");
            socket_close(fd);
            return;
        }
        ...
        if (!ifs.is_open()) {
            send_error(fd, 500,
                std::string("failed to open ") + asset_name);
            socket_close(fd);
            return;
        }

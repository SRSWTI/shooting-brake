...
public:
    ...
    bool compute(
        ...
        uint8_t * mid_conv_data = thread_bufs_[0].mid_conv.data();

        // Keep one OpenMP team alive across all selected experts. The previous
        // implementation entered two parallel regions per expert (gate/up and
        // down), even though every phase has fixed, independent row ranges.
        // Worksharing barriers preserve the original expert order and make each
        // output row single-writer throughout the accumulation.
#ifdef _OPENMP
...
#endif
        {
            for (int e = 0; e < n_cold; ++e) {
                ...
                if (layer.fused_gate_up) {
                    const char * expert =
                        (const char *)layer.gate_up_data + (size_t)eid * layer.gate_up_stride;
                    ...
                    if (layer.gate_up_scale != 1.0f) {
                        ...
                        for (int i = 0; i < n_rows; ++i) {
                            scratch[i] *= layer.gate_up_scale;
                        }
                    }
                } else {
                    const char * gate_expert =
                        (const char *)layer.gate_data + (size_t)eid * layer.gate_stride;
                    const char * up_expert =
                        (const char *)layer.up_data + (size_t)eid * layer.up_stride;
                    const uint8_t * up_inp =
                        inp_conv_up.empty() ? inp_conv_.data() : inp_conv_up.data();
#ifdef _OPENMP
                    #pragma omp for schedule(static)
#endif
                ...
                #pragma omp single
#endif
                dn_from_float(scratch, mid_conv_data, n_ff);

                const char * down_expert =
                    (const char *)layer.down_data + (size_t)eid * layer.down_stride;

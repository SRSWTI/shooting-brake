...
Work proceeds in the Phase 0–10 order from [`../plan.md`](../plan.md). Phases 0 through 7 are complete. The immediate deliverable is:

1. **Phase 8 piecewise CUDA graphs:** add an explicit break around the external B70 provider operation, use dedicated CUDA copy streams/events/pinned buffers to overlap B70 compute with CUDA execution, and remove exposed device-wide synchronization. This is where latency hiding makes the hybrid path production-viable.

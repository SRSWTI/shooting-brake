...
// xe-fuse test: K5_fused — gemm_rmsnorm_partial_cross_entropy (fully-fused epilogue)
//
// Tests GemmRmsNormPartialCrossEntropyFused: GEMM epilogue writes per-tile lse_partial,
// then launch_combine_lse() produces lse[M] without re-reading M×N logits.

#include "xe-fuse/kernels/gemm_rmsnorm_partial_cross_entropy_fused.hpp"
...
using K5F = xe_fuse::GemmRmsNormPartialCrossEntropyFused<>;
using GemmOp = K5F::Gemm;
...
int main(int argc, const char** argv) {
  ...
  auto stride_C = cutlass::make_cute_packed_stride(K5F::StrideC{}, make_shape(M, N, L));
  auto stride_D = cutlass::make_cute_packed_stride(K5F::StrideD{}, make_shape(M, N, L));

  cutlass::DeviceAllocation<K5F::ElementA>     block_A(static_cast<size_t>(M) * K * L);
  cutlass::DeviceAllocation<K5F::ElementB>     block_B(static_cast<size_t>(K) * N * L);
  cutlass::DeviceAllocation<K5F::ElementD>     block_D(static_cast<size_t>(M) * N * L);
  cutlass::DeviceAllocation<K5F::ElementD>     block_ref_D(static_cast<size_t>(M) * N * L);
  cutlass::DeviceAllocation<K5F::ElementScale> block_rstd(static_cast<size_t>(M) * L);
  ...
  size_t partials_size = K5F::get_partials_size(M, N, L);
  cutlass::DeviceAllocation<float> block_lse_partial(partials_size);

  initialize_block(block_A, 2023);
  initialize_block(block_B, 2022);

  std::mt19937 rng(42);
  std::vector<K5F::ElementScale> h_rstd(static_cast<size_t>(M) * L);
  ...
  compat::get_default_queue().memcpy(block_rstd.get(), h_rstd.data(),
                                      h_rstd.size() * sizeof(K5F::ElementScale));
  ...
  auto evt_args = K5F::make_evt_args(block_rstd.get(), M, block_lse_partial.get(), N, L);
  ...
  K5F::launch_combine_lse(q, block_lse_partial.get(), block_lse.get(), M, N, L);
  K5F::launch_select_logits(q, block_D.get(), block_targets.get(),
  ...
  if (opts.verify) {
    ...
    cutlass::reference::device::GemmComplex(
      {M, N, K}, K5F::ElementAcc(1), ref_A, cutlass::ComplexTransform::kNone,
      ref_B, cutlass::ComplexTransform::kNone, K5F::ElementAcc(0),
      ref_C, ref_D, K5F::ElementAcc(0), L, M * K, K * N, M * N, M * N);
    compat::wait();

    // Apply rstd scaling to reference
    {
      ...
      q.parallel_for(sycl::range<1>(static_cast<size_t>(M) * N * L),
        [=](sycl::id<1> idx) {
          ...
          ref_ptr[i] = static_cast<K5F::ElementD>(val * s);
        });
    }
    compat::wait();
    ...
    bool d_passed = cutlass::reference::device::BlockCompareRelativelyEqual(
      ...
      static_cast<K5F::ElementD>(0.05f), static_cast<K5F::ElementD>(0.05f));
  ...
  if (opts.iterations > 0) {
    ...
    printf("xe-fuse K5_fused (GEMM+RmsNorm+LSE+SelectLogits fused): [%4.3f]TFlop/s  (%6.4f)ms\n",

...
- A CUDA-resident subset of routed experts stays on the 5090; qualified cold/overflow experts reside on B70 to increase usable model/KV capacity. This is stated as an *intent* for the placement policy, and as of 2026-08-08 no shipped policy achieves it: `LayerSubsetPolicy` and `AllOutPolicy` select by expert **index**, not by measured route count, and the index-based CUDA set absorbs 3.3% of routes against 3.1% for a random set. The route histogram now exists (`SHOOTING_BRAKE_ROUTE_STATS=1`, profile in `profiles/`) and measures the model's skew at 1.39× over uniform — enough that a frequency-ranked set absorbs 9.0% at the same budget, not enough to call any subset "hot". A calibrated policy is pending; until it lands, read "cold/overflow" as an ownership label rather than a frequency claim.

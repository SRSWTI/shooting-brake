...
**Status: Phase 8a COMPLETE — async B70 overlap gate passed on 2026-08-05.** The B70 provider's SYCL `issue()` already enqueues the kernel asynchronously (confirmed by reading the Phase 1 C++ source). Phase 8a exploits this by splitting the Python binding into separate `issue()` / `take()` methods and reordering `forward_modular`: issue B70 **before** CUDA routed-expert compute, take **after** — so the B70 kernel (~44µs) overlaps with CUDA's forward_modular (~100µs+). Pre-allocated pinned host buffers (`pin_memory=True, device="cpu"`) enable DMA D2H/H2D. `phase8/async_overlap_test.py` verifies three-way parity: all-CUDA, async-B70, sync-B70 all produce identical tokens `[271, 760, 7308, 1238, 220, 19, 16, 369]`; async and sync B70 paths agree (same kernel, same data). Controlled by `SHOOTING_BRAKE_B70_ASYNC` (default: `"1"` when `B70_DEVICE=1`, `"0"` for synchronous Phase 7 reference). Phase 8b (piecewise CUDA graphs) is deferred — the NVFP4 model has good eager performance and standard vLLM graph capture can't span the cross-device break.
...
### Phase 8.5 — VRAM surgery: physically remove B70 expert weights from CUDA

**Status: COMPLETE — VRAM surgery gate passed on 2026-08-05.** B70-owned expert weights are physically sliced out of CUDA weight tensors on the first `forward_modular` call, freeing **6.445 GB** of VRAM (measured: layer 0 pre-surgery 23.48 GB → layer 31 post-surgery 17.03 GB). The freed VRAM is available for KV cache, increasing concurrent request capacity.

The surgery runs lazily per-layer after vLLM's `process_weights_after_loading`. It slices every per-expert tensor (`w13_weight`, `w2_weight`, `w13_weight_scale`, `w2_weight_scale`, `w13_weight_scale_2`, `w2_weight_scale_2`, `w13_input_scale`, `w2_input_scale`) to contain only CUDA-owned experts, then updates the `FusedMoEQuantConfig`'s internal `FusedMoEQuantDesc` references so the kernel's `@property` delegates see the smaller tensors. A global→local remap tensor translates `topk_ids` for the CUDA kernel: CUDA routes → compact local IDs, B70 routes → expert 0 with zero weight (producing zero contribution; real results come from the B70 device).

Investigation confirmed no NVFP4 MoE backend in vLLM supports `expert_map` for EP=1 single-device setups — the mechanism is exclusive to multi-rank EP with NCCL all2all. The surgery bypasses this limitation by working entirely within the adapter's `forward_modular` override.

`phase8/vram_surgery_test.py` passes with exact token parity under `SHOOTING_BRAKE_VRAM_SURGERY=1`. Controlled by env vars: requires `B70_DEVICE=1` and `HYBRID=1` (B70 experts are removed from CUDA and must be computed on the B70 device).

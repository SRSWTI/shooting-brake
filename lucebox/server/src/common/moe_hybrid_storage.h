...
struct MoeHybridStorage {
    ...
    // Optional third tier: cold expert bytes remain in the GGUF and are read
    // through a bounded pinned staging pool before synchronous, completion-safe
    // publication into spare GPU cache slots.
    std::unique_ptr<MoeSsdExpertStore> ssd_store;

    bool has_ssd_store() const { return ssd_store != nullptr; }
...
// Load all selected nonresident experts from the GGUF-backed SSD tier into
// spare GPU slots. Publication happens only after every weight upload for that
// expert completes. Returns false rather than dropping an expert.
...
// Build hybrid storage from file and transfer mmap ownership to the storage.
// mmap_fd is duplicated for explicit SSD reads when nonnegative; the caller
// may close its descriptor after this function returns.
// This variant populates per-layer file regions for streaming and SSD caching.

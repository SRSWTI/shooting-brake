...
static bool run_case(
    ...
    if (duplicate_ids) {
        for (int token = 0; token < width; ++token) {
            ids_h[(size_t) token * top_k + 4] =
                ids_h[(size_t) token * top_k + 2];
        }
    }
...
static int run_child(const char * mode, const char * output_path) {
    ...
    // Q8_0 batches wider than the MMVQ ceiling use CUDA's synchronous
    // MUL_MAT_ID fallback on Blackwell Consumer. Hybrid offload supplies
    // valid zero-weight dummy routes that can duplicate another expert slot.
    // Preserve every route instead of assuming per-token IDs are unique.
    ok = run_case(backend, GGML_TYPE_Q8_0, 32, false, true, output, true) && ok;
    output.close();
...
int main(int argc, char ** argv) {
    ...
    {
        constexpr int duplicate_width = 32;
        const size_t case_bytes =
            (size_t) 128 * 8 * duplicate_width * sizeof(float);
        if (output_parity) {
            output_parity = compare_case_outputs(
                legacy, grouped, offset, case_bytes, true);
            if (!output_parity) {
                std::fprintf(stderr, "duplicate-ID output mismatch type=q8_0 width=%d\n",
                             duplicate_width);
            }
        }
        compared_bytes += case_bytes;
    ...
    output_parity = output_parity && offset == legacy.size() && compared_cases == 51;

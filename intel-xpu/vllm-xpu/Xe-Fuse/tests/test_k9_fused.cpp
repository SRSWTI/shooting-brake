...
//
// Tests GemmPartialCrossEntropyFused: GEMM epilogue writes per-tile lse_partial,
// then launch_combine_lse() produces lse[M] without re-reading M×N logits.

#include "xe-fuse/kernels/gemm_partial_cross_entropy_fused.hpp"
...
int main(int argc, const char** argv) {
  ...
  size_t partials_size = K9F::get_partials_size(M, N, L);
  ...
  auto evt_args = K9F::make_evt_args(block_lse_partial.get(), M, N, L);
  ...
  K9F::launch_combine_lse(q, block_lse_partial.get(), block_lse.get(), M, N, L);

...
bool Qwen35MoeBackend::init() {
    ...
    pipe_state_->expert_compute =
        !target_weights().moe_hybrid->has_ssd_store() &&
        (std::getenv("DFLASH_DROP_COLD") == nullptr);
...
bool Qwen35MoeBackend::load_target_model(ggml_backend_t backend, TargetWeights & out) {
    ...
    // Open GGUF again to get tensor file offsets and mmap the data.
    {
        ...
        // Transfer mmap ownership out of the RAII wrapper. Hybrid storage owns
        // the mapping; its SSD tier duplicates the descriptor before we close it.
        ...
        int cache_slots = 0;
        if (const char * cs = std::getenv("DFLASH_QWEN35MOE_CACHE_SLOTS")) {
            cache_slots = std::max(0, std::atoi(cs));
        } else if (cache_slots_ >= 0) {
            cache_slots = cache_slots_;
        }
        if (ssd_enabled) cache_slots = std::max(cache_slots, out.n_expert_used);
        const bool storage_ok = build_moe_hybrid_storage_from_file_with_mmap(
    ...
    const char * cold_tier = out.moe_hybrid->has_ssd_store() ? "GGUF" : "RAM";
    std::printf("[qwen35moe] hybrid storage ready: total_hot=%d (%.2f GiB VRAM) total_cold=%d (%.2f GiB %s) source=%s\n",
...
bool Qwen35MoeBackend::rebuild_hybrid_from_placement(const MoeHybridPlacement & placement,
    ...
    gguf_context * gctx = gguf_init_from_file(cfg_.target_path, gip);
    if (!gctx) {
        err = "gguf reinit failed";
        return false;
    }
    GgufMmap mapped_file;
    std::string mmap_error;
    if (!mapped_file.open(cfg_.target_path, mmap_error)) {
        gguf_free(gctx);
        err = "mmap failed: " + mmap_error;
        return false;
    }
    const size_t file_size = mapped_file.size();
    const void * mmap_addr = mapped_file.data();
    const size_t data_start = gguf_get_data_offset(gctx);
    const auto * file_bytes = static_cast<const uint8_t *>(mmap_addr);
    ...
    for (int il = 0; il < out.n_layer; ++il) {
        ...
        auto find_tensor_data = [&](const char * suffix) -> ExpertTensorFileData {
            ...
            if (tid < 0) return {};
            const size_t offset = data_start + gguf_get_tensor_offset(gctx, tid);
            const size_t size = gguf_get_tensor_size(gctx, tid);
            if (offset + size > file_size) return {};
            return {file_bytes + offset, size};
        };
        layer_file_data[(size_t)il].gate_exps = find_tensor_data("ffn_gate_exps");
        layer_file_data[(size_t)il].up_exps = find_tensor_data("ffn_up_exps");
        layer_file_data[(size_t)il].down_exps = find_tensor_data("ffn_down_exps");
    ...
    GgufMmap::OwnedRegion region = mapped_file.release();
    ...
    int cache_slots = cache_slots_ >= 0 ? cache_slots_ : 0;
    if (ssd_enabled) cache_slots = std::max(cache_slots, out.n_expert_used);

    const bool ok = build_moe_hybrid_storage_from_file_with_mmap(
        hybrid_cfg, backend, placement, layer_descs, layer_file_data,
        mmap_addr, file_size, region.fd, *hybrid, &err, cache_slots);
#if !defined(_WIN32)
...
GenerateResult Qwen35MoeBackend::generate_impl(const GenerateRequest & req,
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        for (int chunk_start = 0; chunk_start < prompt_len; chunk_start += prefill_chunk) {
            ...
            bool ffn_ok = false;
            if (ssd_mode) {
                // A prompt chunk can touch more experts than the bounded cache.
                // Keep each token's exact routes resident through its FFN before
                // allowing the next token to reuse those cache slots.
                ffn_batch_out.assign((size_t)hidden * (size_t)chunk_len, 0.0f);
                std::vector<float> single_out;
                ffn_ok = true;
                for (int ti = 0; ti < chunk_len; ++ti) {
                    const int32_t * tok_sel =
                    ...
                    if (!dflash::common::moe_hybrid_ssd_cache_selected(
                            *target_weights().moe_hybrid, il, tok_sel,
                            n_expert_used, target_backend(), &ffn_error) ||
                        !eval_moe_hybrid_ffn_single(
                            target_backend(), chunk_cfg, chunk_desc, storage, cpu_be,
                            chunk_post.data() + (size_t)ti * (size_t)hidden,
                            tok_sel, tok_wts, n_expert_used, single_out,
                            nullptr, &ffn_error)) {
                        ffn_ok = false;
                        break;
                    }
                    std::memcpy(
                        ffn_batch_out.data() + (size_t)ti * (size_t)hidden,
                        single_out.data(), sizeof(float) * (size_t)hidden);
                }
            } else {
                // RAM-backed cold experts use the shared CPU/remote expert
                // runtime and retain the existing batched split path.
            ...
            if (!ffn_ok) {
                std::fprintf(stderr, "[qwen35moe] SSD prefill FFN failed: %s\n",
                             ffn_error.c_str());
                result.fail(GenerateErrorCode::BackendSpecific, "prefill_ssd_ffn");
                step_graph_destroy(prefill_sg);
                if (ffn_hot_alloc) ggml_gallocr_free(ffn_hot_alloc);
                if (ffn_cold_alloc) ggml_gallocr_free(ffn_cold_alloc);
                cleanup_graphs();
...
bool Qwen35MoeBackend::hybrid_forward_batch(
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (target_weights().moe_hybrid->has_ssd_store()) {
            // A prompt can touch more unique experts than the bounded GPU cache.
            // Preserve exactness by processing prompt tokens in order: load that
            // token's routes, compute it while resident, then reuse slots.
            ...
            for (int ti = 0; ti < n_tokens; ++ti) {
                ...
                const float * token_weights =
                    chunk_weights.data() + (size_t)ti * (size_t)n_expert_used;
                if (!dflash::common::moe_hybrid_ssd_cache_selected(
                        *target_weights().moe_hybrid, il, token_ids,
                        n_expert_used, target_backend(), &ssd_error)) {
                    std::fprintf(stderr, "[qwen35moe] SSD prefill load failed: %s\n",
                                 ssd_error.c_str());
                    ffn_ok = false;
                    break;
                }
                if (!eval_moe_hybrid_ffn_single(
                        ...
                        chunk_post.data() + (size_t)ti * (size_t)hidden,
                        token_ids, token_weights, n_expert_used,
                        single_out, nullptr)) {
                    ffn_ok = false;
                    break;
                }
            ...
            // RAM-backed Spark can warm all routes in a verify batch and retain
            // the existing batched fast path.
            ...
            if (storage.cache_slots > 0 && !storage.cold_expert_ids.empty()) {
                for (int i = 0; i < n_route_slots; ++i) {
                    dflash::common::moe_hybrid_cache_swap_in(
                        storage, chunk_selected[(size_t)i], target_backend());
                }
            }

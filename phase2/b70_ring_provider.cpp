...
struct Options final {
  ...
  std::string bank_path = "phase1/expert_bank.bin";
  bool quiet = false;
};
...
bool receive_bootstrap(int socket, Bootstrap* bootstrap,
...
bool receive_control(int socket, Control* control) noexcept {
...
int serve(sb2::SharedRing& ring, sb1::B70Provider& provider, int control_socket,
  ...
  for (;;) {
    ...
    if (claim_status == sb2::RingStatus::ok) {
      ...
      for (std::uint32_t index = 0; index < positions; ++index) {
        if (claim.request.remote_mask[index] != 0U) {
          local_ids[index] = claim.request.canonical_ids[index];
    ...
    if ((waits[0].revents & POLLIN) != 0) {
      ...
      if (!receive_control(control_socket, &control) ||
          !exact_magic(control.magic, kControlMagic) ||
          control.major != kBootstrapMajor ||
          control.minor != kBootstrapMinor || control.bytes != sizeof(control) ||
          control.command != 1U || control.reserved0 != 0U ||
  ...
  ShutdownReply reply{};
  copy_magic(reply.magic, kShutdownMagic);
  reply.major = kBootstrapMajor;
  reply.minor = kBootstrapMinor;
...
int run(const Options& options) {
  ...
  if (!receive_bootstrap(connection.get(), &bootstrap, &descriptors) ||
      !exact_magic(bootstrap.magic, kBootstrapMagic) ||
      bootstrap.major != kBootstrapMajor ||
      bootstrap.minor != kBootstrapMinor || bootstrap.bytes != sizeof(bootstrap) ||
  ...
  const sb1::ProviderStatus load = provider.load(options.bank_path, config);

  StartupReply startup{};
  copy_magic(startup.magic, kStartupMagic);
  ...
  const int service_status = serve(ring, provider, connection.get(),

...
extern "C" int coli_cuda_init(const int *devices, int count) {
    ...
    /* Dense/state reservation precedes expert-tier initialization in qwen36.
     * The tier therefore calls init a second time. Reusing the exact device set
     * must be a no-op: resetting g_ctx here would leak streams and every dense
     * allocation and invalidate all tensor handles. */
    if(g_nctx){
        if(g_nctx!=count) return 0;
        for(int i=0;i<count;i++) if(g_ctx[i].device!=devices[i]) return 0;
        return 1;
    }
    g_current_device=-1;
...
extern "C" int coli_cuda_tensor_upload(ColiCudaTensor **tensor,
    ...
    if (!tensor) return 0;
    if(fmt==4&&(g_upload_gs<=0||(g_upload_gs&1))) return 0;
    if (*tensor) {
...
extern "C" int coli_cuda_expert_group_issue(ColiCudaTensor *const *gates,
    ...
    ColiCudaTensor *first=gates[0];
    if (!first) return 0;
    int device=first->device,D=first->I,I=first->O,total=0,max_rows=0,all_s4=1,all_q4=1,any_g4=0;
    GroupDesc host[64];
    for(int c=0;c<count;c++){
        ColiCudaTensor *g=gates[c],*u=ups[c],*d=downs[c];
        if(!g||!u||!d||rows[c]<1||g->device!=device||u->device!=device||d->device!=device||
...
extern "C" int coli_cuda_expert_group_resident_issue(ColiCudaTensor *const *gates,
    ...
    for(int c=0;c<count;c++){
        ...
        host[c]={g->weights,u->weights,d->weights,g->scales,u->scales,d->scales,
                 ...
                 g->gs,u->gs,d->gs};
        all_s4&=(g->fmt==2||g->fmt==4)&&(u->fmt==2||u->fmt==4)&&
                (d->fmt==2||d->fmt==4);
        if((g->fmt==4&&g->gs%2)||(u->fmt==4&&u->gs%2)||(d->fmt==4&&d->gs%2))
            return 0;
        total++;
    }
    if(!all_s4) return 0;
    ...
    int grouped=0;
    for(int c=0;c<count;c++)
        grouped|=host[c].gf==4||host[c].uf==4||host[c].df==4;
    if(grouped){
        grouped_hidden_g4_dual<<<hg,256,0,ctx->stream>>>(ctx->gate,ctx->up,ctx->x,dev,I,D);
        grouped_down_g4<<<og,256,0,ctx->stream>>>(ctx->y,ctx->gate,dev,D,I);
    }else{
...
/* ==== Qwen3.6 persistent state-owner primitives ========================= */
__global__ static void qwen_rmsnorm_rows(float *y,const float *x,const float *w,
                                          int D,float eps){
    const float *xr=x+(size_t)blockIdx.x*D;
    float *yr=y+(size_t)blockIdx.x*D;
    __shared__ float sh[256];
    float a=0.f;
    for(int i=threadIdx.x;i<D;i+=blockDim.x) a+=xr[i]*xr[i];
    sh[threadIdx.x]=a; __syncthreads();
    for(int n=blockDim.x/2;n;n>>=1){
        if(threadIdx.x<n) sh[threadIdx.x]+=sh[threadIdx.x+n];
        __syncthreads();
    }
    float r=rsqrtf(sh[0]/D+eps);
    for(int i=threadIdx.x;i<D;i+=blockDim.x) yr[i]=xr[i]*r*(1.f+w[i]);
}

__global__ static void embedding_rows(float *out,const int *ids,const void *weights,
                                       const float *scales,int fmt,int I,size_t rb,
                                       int gs,int ng){
    int s=blockIdx.x,id=ids[s];
    float *dst=out+(size_t)s*I;
    size_t row=(size_t)id*rb;
    for(int i=threadIdx.x;i<I;i+=blockDim.x)
        dst[i]=weight_at(weights,fmt,row,i)*absorb_scale(scales,fmt,gs,ng,id,i);
}

__global__ static void qwen_qk_prepare(float *q,float *k,const float *qn,
                                       const float *kn,float *kc,float *vc,
                                       const float *v,int S,int pos_base,int cap,
                                       int H,int KV,int hd,int qdim,int khd,
                                       int rotary,float eps,float theta){
    int row=blockIdx.x;
    if(row<S*H){
        int s=row/H,h=row%H;
        float *p=q+(size_t)s*H*qdim+(size_t)h*qdim;
        __shared__ float sh[256];
        float ss=0.f;
        for(int d=threadIdx.x;d<hd;d+=blockDim.x) ss+=p[d]*p[d];
        ...
        for(int d=threadIdx.x;d<hd;d+=blockDim.x) p[d]*=r*(1.f+qn[d]);
        __syncthreads();
        int half=rotary/2,pos=pos_base+s;
        for(int d=threadIdx.x;d<half;d+=blockDim.x){
            float inv=__powf(theta,-2.f*d/rotary),ang=pos*inv;
            float cs=__cosf(ang),sn=__sinf(ang),a=p[d],b=p[half+d];
            p[d]=a*cs-b*sn; p[half+d]=b*cs+a*sn;
        }
        return;
    }
    row-=S*H;
    if(row>=S*KV) return;
    int s=row/KV,h=row%KV,pos=pos_base+s;
    float *p=k+(size_t)s*KV*khd+(size_t)h*khd;
    __shared__ float sh[256];
    float ss=0.f;
    for(int d=threadIdx.x;d<khd;d+=blockDim.x) ss+=p[d]*p[d];
    sh[threadIdx.x]=ss; __syncthreads();
    for(int n=blockDim.x/2;n;n>>=1){
        if(threadIdx.x<n) sh[threadIdx.x]+=sh[threadIdx.x+n];
        __syncthreads();
    }
    float r=rsqrtf(sh[0]/khd+eps);
    for(int d=threadIdx.x;d<khd;d+=blockDim.x) p[d]*=r*(1.f+kn[d]);
    __syncthreads();
    int half=rotary/2;
    for(int d=threadIdx.x;d<half;d+=blockDim.x){
        float inv=__powf(theta,-2.f*d/rotary),ang=pos*inv;
        float cs=__cosf(ang),sn=__sinf(ang),aa=p[d],bb=p[half+d];
        p[d]=aa*cs-bb*sn; p[half+d]=bb*cs+aa*sn;
    }
    __syncthreads();
    for(int d=threadIdx.x;d<khd;d+=blockDim.x){
        kc[((size_t)h*cap+pos)*khd+d]=p[d];
        vc[((size_t)h*cap+pos)*khd+d]=v[(size_t)s*KV*khd+(size_t)h*khd+d];
    }
}

__global__ static void qwen_gqa_rows(float *out,const float *q,const float *kc,
                                     const float *vc,int S,int pos_base,int cap,
                                     int H,int KV,int hd,int qdim,int khd){
    int row=blockIdx.x,s=row/H,h=row%H,kvh=h/(H/KV),last=pos_base+s;
    const float *qp=q+(size_t)s*H*qdim+(size_t)h*qdim;
    __shared__ float red[256];
    __shared__ float scalar;
    float mx=-3.402823466e38f;
    for(int t=0;t<=last;t++){
        float dot=0.f;
        const float *kp=kc+((size_t)kvh*cap+t)*khd;
        for(int d=threadIdx.x;d<khd;d+=blockDim.x) dot+=qp[d]*kp[d];
        red[threadIdx.x]=dot; __syncthreads();
        for(int n=blockDim.x/2;n;n>>=1){
            if(threadIdx.x<n) red[threadIdx.x]+=red[threadIdx.x+n];
            __syncthreads();
        }
        if(threadIdx.x==0){ float z=red[0]*rsqrtf((float)hd); if(z>mx) mx=z; }
        __syncthreads();
    }
    if(threadIdx.x==0) scalar=mx;
    __syncthreads(); mx=scalar;
    float sm=0.f;
    for(int t=0;t<=last;t++){
        float dot=0.f;
        const float *kp=kc+((size_t)kvh*cap+t)*khd;
        ...
        if(threadIdx.x==0) sm+=expf(red[0]*rsqrtf((float)hd)-mx);
        __syncthreads();
    }
    if(threadIdx.x==0) scalar=sm;
    __syncthreads(); sm=scalar;
    ...
    for(int d=threadIdx.x;d<hd;d+=blockDim.x){
        ...
        for(int t=0;t<=last;t++){
            float dot=0.f;
            const float *kp=kc+((size_t)kvh*cap+t)*khd;
            for(int kk=0;kk<khd;kk++) dot+=qp[kk]*kp[kk];
            float p=expf(dot*rsqrtf((float)hd)-mx)/sm;
            acc+=p*vc[((size_t)kvh*cap+t)*khd+d];
        }
        float gate=(qdim>hd)?q[(size_t)s*H*qdim+(size_t)h*qdim+hd+d]:0.f;
        dst[d]=acc/(1.f+expf(-gate));
    }
}

__global__ static void qwen_conv_seq(float *co,const float *qkv,const float *cw,
                                     float *state,int S,int C,int K){
    int c=blockIdx.x*blockDim.x+threadIdx.x;
    if(c>=C) return;
    float *ring=state+(size_t)c*(K-1);
    const float *w=cw+(size_t)c*K;
    for(int s=0;s<S;s++){
        float v=0.f;
        for(int j=0;j<K-1;j++) v+=w[j]*ring[j];
        float cur=qkv[(size_t)s*C+c];
        v+=w[K-1]*cur;
        co[(size_t)s*C+c]=v/(1.f+expf(-v));
        for(int j=0;j<K-2;j++) ring[j]=ring[j+1];
        ring[K-2]=cur;
    }
}

__global__ static void qwen_delta_params(float *beta,float *decay,const float *b,
                                         const float *a,const float *dt,
                                         const float *alog,int S,int H){
    int i=blockIdx.x*blockDim.x+threadIdx.x;
    if(i>=S*H) return;
    int h=i%H;
    beta[i]=1.f/(1.f+expf(-b[i]));
    float z=a[i]+dt[h];
    float sp=z>20.f?z:log1pf(expf(z));
    decay[i]=expf(-expf(alog[h])*sp);
}

__global__ static void qwen_delta_recur(float *out,const float *co,const float *z,
                                        const float *beta,const float *decay,
                                        const float *nw,float *rec,int S,int VH,
                                        int KH,int KD,int VD,int C,float eps){
    int h=blockIdx.x,rep=VH/KH,kh=h/rep,key_total=KH*KD;
    float *Sh=rec+(size_t)h*KD*VD;
    __shared__ float qr[256],kr[256],norms[2];
    for(int s=0;s<S;s++){
        const float *q=co+(size_t)s*C+(size_t)kh*KD;
        const float *k=co+(size_t)s*C+key_total+(size_t)kh*KD;
        const float *v=co+(size_t)s*C+2*key_total+(size_t)h*VD;
        float qss=0.f,kss=0.f;
        for(int d=threadIdx.x;d<KD;d+=blockDim.x){ qss+=q[d]*q[d]; kss+=k[d]*k[d]; }
        qr[threadIdx.x]=qss; kr[threadIdx.x]=kss; __syncthreads();
        for(int n=blockDim.x/2;n;n>>=1){
            if(threadIdx.x<n){ qr[threadIdx.x]+=qr[threadIdx.x+n]; kr[threadIdx.x]+=kr[threadIdx.x+n]; }
            __syncthreads();
        }
        if(threadIdx.x==0){ norms[0]=rsqrtf(qr[0]+1e-6f)*rsqrtf((float)KD);
                            norms[1]=rsqrtf(kr[0]+1e-6f); }
        __syncthreads();
        float ov=0.f;
        if(threadIdx.x<VD){
            int vv=threadIdx.x;
            ...
            for(int kk=0;kk<KD;kk++){
                float *cell=Sh+(size_t)kk*VD+vv;
                *cell*=decay[(size_t)s*VH+h];
                kv+=k[kk]*norms[1]*(*cell);
            }
            float dl=(v[vv]-kv)*beta[(size_t)s*VH+h];
            for(int kk=0;kk<KD;kk++)
                Sh[(size_t)kk*VD+vv]+=k[kk]*norms[1]*dl;
            for(int kk=0;kk<KD;kk++)
                ov+=q[kk]*norms[0]*Sh[(size_t)kk*VD+vv];
            out[((size_t)s*VH+h)*VD+vv]=ov;
            qr[threadIdx.x]=ov*ov;
        }else qr[threadIdx.x]=0.f;
        __syncthreads();
        for(int n=blockDim.x/2;n;n>>=1){
            if(threadIdx.x<n) qr[threadIdx.x]+=qr[threadIdx.x+n];
            __syncthreads();
        }
        float r=rsqrtf(qr[0]/VD+eps);
        if(threadIdx.x<VD){
            size_t oi=((size_t)s*VH+h)*VD+threadIdx.x;
            float zz=z[oi],sw=zz/(1.f+expf(-zz));
            out[oi]=out[oi]*r*nw[threadIdx.x]*sw;
        }
        __syncthreads();
    }
}

__global__ static void qwen_router_select(float *logits,const float *bias,int S,int E,
                                          int K,int NG,int KG,int *ids,float *weights){
    int s=blockIdx.x;
    if(threadIdx.x) return;
    float *p=logits+(size_t)s*E;
    float mx=-3.402823466e38f;
    for(int e=0;e<E;e++){ if(bias) p[e]+=bias[e]; if(p[e]>mx) mx=p[e]; }
    float sum=0.f;
    ...
    for(int g=0;g<NG;g++){
        ...
        for(int e=g*per;e<(g+1)*per;e++){
            float v=p[e]; if(v>b1){b2=b1;b1=v;}else if(v>b2)b2=v;
        }
        gscore[g]=b1+b2;
    }
    if(NG>1) for(int pick=0;pick<KG;pick++){
        int bg=-1; float bv=-1.f;
        for(int g=0;g<NG;g++) if(!gkeep[g]&&gscore[g]>bv){bv=gscore[g];bg=g;}
        if(bg>=0) gkeep[bg]=1;
    }
    for(int kk=0;kk<K;kk++){
        int best=-1; float bv=-1.f;
        for(int e=0;e<E;e++){
            if(NG>1&&!gkeep[e/per]) continue;
            int used=0;
            for(int j=0;j<kk;j++) if(ids[(size_t)s*K+j]==e){used=1;break;}
            if(!used&&p[e]>bv){bv=p[e];best=e;}
        }
        ids[(size_t)s*K+kk]=best;
        weights[(size_t)s*K+kk]=bv;
    }
    float z=0.f; for(int k=0;k<K;k++) z+=weights[(size_t)s*K+k];
    if(z>0.f) for(int k=0;k<K;k++) weights[(size_t)s*K+k]/=z;
}

__global__ static void qwen_shared_gate(float *out,const float *shared,const float *x,
                                        const float *w,int D){
    int s=blockIdx.x;
    __shared__ float sh[256],gate;
    float a=0.f;
    for(int d=threadIdx.x;d<D;d+=blockDim.x) a+=x[(size_t)s*D+d]*w[d];
    sh[threadIdx.x]=a; __syncthreads();
    for(int n=blockDim.x/2;n;n>>=1){
        if(threadIdx.x<n) sh[threadIdx.x]+=sh[threadIdx.x+n];
        __syncthreads();
    }
    if(threadIdx.x==0) gate=1.f/(1.f+expf(-sh[0]));
    __syncthreads();
    for(int d=threadIdx.x;d<D;d+=blockDim.x)
        out[(size_t)s*D+d]+=gate*shared[(size_t)s*D+d];
}

__global__ static void deterministic_argmax(const float *x,int n,int *idx,float *val){
    if(threadIdx.x||blockIdx.x) return;
    int bi=0; float bv=x[0];
    for(int i=1;i<n;i++) if(x[i]>bv){bv=x[i];bi=i;}
    *idx=bi; if(val)*val=bv;
}

extern "C" int coli_cuda_pipe_zero(int device,void *dst,size_t bytes){
    DeviceContext *ctx=find_ctx(device);
    return dst&&select_ctx(ctx)&&cuda_ok(cudaMemset(dst,0,bytes),"pipe zero");
}
extern "C" int coli_cuda_pipe_copy(int device,void *dst,const void *src,size_t bytes){
    DeviceContext *ctx=find_ctx(device);
    return dst&&src&&select_ctx(ctx)&&
           cuda_ok(cudaMemcpy(dst,src,bytes,cudaMemcpyDeviceToDevice),"pipe copy");
}
extern "C" int coli_cuda_pipe_embedding(ColiCudaTensor *t,float *out,
                                        const int *ids_host,int S){
    if(!t||!out||!ids_host||S<1) return 0;
    DeviceContext *ctx=find_ctx(t->device); if(!select_ctx(ctx)) return 0;
    int *ids=(int*)coli_cuda_pipe_scratch(t->device,26,(size_t)S*sizeof(int));
    if(!ids||!cuda_ok(cudaMemcpy(ids,ids_host,(size_t)S*sizeof(int),cudaMemcpyHostToDevice),
                      "embedding ids")) return 0;
    embedding_rows<<<S,256>>>(out,ids,t->weights,t->scales,t->fmt,t->I,
                              row_bytes(t->fmt,t->I),t->gs,t->ng);
    return cuda_ok(cudaGetLastError(),"embedding gather");
}
extern "C" int coli_cuda_qwen_rmsnorm(int device,float *y,const float *x,
                                      const float *w,int S,int D,float eps){
    DeviceContext *ctx=find_ctx(device);
    if(!x||!y||!w||S<1||D<1||!select_ctx(ctx)) return 0;
    qwen_rmsnorm_rows<<<S,256>>>(y,x,w,D,eps);
    return cuda_ok(cudaGetLastError(),"qwen rmsnorm");
}
extern "C" int coli_cuda_qwen_gqa(int device,float *out,float *q,float *k,
        const float *v,const float *qn,const float *kn,float *kc,float *vc,
        int S,int pos,int cap,int H,int KV,int hd,int qdim,int khd,int rotary,
        float eps,float theta){
    DeviceContext *ctx=find_ctx(device);
    if(!out||!q||!k||!v||!qn||!kn||!kc||!vc||S<1||pos<0||pos+S>cap||
       H<1||KV<1||H%KV||hd<1||khd!=hd||qdim<hd||rotary<2||rotary>hd||
       (rotary&1)||!select_ctx(ctx)) return 0;
    qwen_qk_prepare<<<S*H+S*KV,256>>>(q,k,qn,kn,kc,vc,v,S,pos,cap,H,KV,hd,
                                      qdim,khd,rotary,eps,theta);
    qwen_gqa_rows<<<S*H,256>>>(out,q,kc,vc,S,pos,cap,H,KV,hd,qdim,khd);
    return cuda_ok(cudaGetLastError(),"qwen gqa");
}
extern "C" int coli_cuda_qwen_deltanet(int device,float *out,const float *qkv,
        const float *z,const float *b,const float *a,const float *cw,
        const float *dt,const float *alog,const float *nw,float *conv,float *rec,
        int S,int VH,int KH,int KD,int VD,int CK,int C,float eps){
    DeviceContext *ctx=find_ctx(device);
    if(!out||!qkv||!z||!b||!a||!cw||!dt||!alog||!nw||!conv||!rec||S<1||
       VH<1||KH<1||VH%KH||KD<1||VD<1||VD>256||CK<2||
       C!=2*KH*KD+VH*VD||!select_ctx(ctx)) return 0;
    size_t rows=(size_t)S*VH;
    float *co=coli_cuda_pipe_scratch(device,20,(size_t)S*C*sizeof(float));
    float *beta=coli_cuda_pipe_scratch(device,21,rows*sizeof(float));
    float *decay=coli_cuda_pipe_scratch(device,22,rows*sizeof(float));
    if(!co||!beta||!decay) return 0;
    qwen_conv_seq<<<(C+255)/256,256>>>(co,qkv,cw,conv,S,C,CK);
    qwen_delta_params<<<(unsigned)((rows+255)/256),256>>>(beta,decay,b,a,dt,alog,S,VH);
    qwen_delta_recur<<<VH,256>>>(out,co,z,beta,decay,nw,rec,S,VH,KH,KD,VD,C,eps);
    return cuda_ok(cudaGetLastError(),"qwen deltanet");
}
extern "C" int coli_cuda_qwen_router(ColiCudaTensor *gate,const float *bias,
        const float *x,int S,int K,int NG,int KG,int *ids,float *weights,
        int *ids_host,float *weights_host,float *logits_host){
    if(!gate||!x||!ids||!weights||!ids_host||!weights_host||S<1||K<1||
       K>32||K>gate->O||NG<1||NG>256||KG<1||KG>NG||gate->O%NG||
       K>KG*(gate->O/NG)||(gate->fmt==4&&(gate->gs<=0||(gate->gs&1)))) return 0;
    DeviceContext *ctx=find_ctx(gate->device); if(!select_ctx(ctx)) return 0;
    float *logits=coli_cuda_pipe_scratch(gate->device,23,(size_t)S*gate->O*sizeof(float));
    if(!logits) return 0;
    quant_matmul<<<dim3((unsigned)gate->O,(unsigned)S),256>>>(logits,x,gate->weights,
        gate->scales,gate->fmt,S,gate->I,gate->O,row_bytes(gate->fmt,gate->I),
        gate->gs,gate->ng);
    qwen_router_select<<<S,1>>>(logits,bias,S,gate->O,K,NG,KG,ids,weights);
    size_t rb=(size_t)S*K;
    if(!cuda_ok(cudaGetLastError(),"qwen router")||
       !cuda_ok(cudaMemcpy(ids_host,ids,rb*sizeof(int),cudaMemcpyDeviceToHost),
                "qwen router ids")||
       !cuda_ok(cudaMemcpy(weights_host,weights,rb*sizeof(float),cudaMemcpyDeviceToHost),
                "qwen router weights")) return 0;
    if(logits_host&&!cuda_ok(cudaMemcpy(logits_host,logits,(size_t)S*gate->O*sizeof(float),
                                        cudaMemcpyDeviceToHost),"qwen router logits"))
        return 0;
    return 1;
}
extern "C" int coli_cuda_qwen_shared_gate_add(int device,float *out,
        const float *shared,const float *x,const float *w,int S,int D){
    DeviceContext *ctx=find_ctx(device);
    if(!out||!shared||!x||!w||S<1||D<1||!select_ctx(ctx)) return 0;
    qwen_shared_gate<<<S,256>>>(out,shared,x,w,D);
    return cuda_ok(cudaGetLastError(),"qwen shared gate");
}
extern "C" int coli_cuda_pipe_argmax(int device,const float *logits,int n,
                                     int *index_host,float *value_host){
    DeviceContext *ctx=find_ctx(device);
    if(!logits||n<1||!index_host||!select_ctx(ctx)) return 0;
    int *di=(int*)coli_cuda_pipe_scratch(device,24,sizeof(int));
    float *dv=coli_cuda_pipe_scratch(device,25,sizeof(float));
    if(!di||!dv) return 0;
    deterministic_argmax<<<1,1>>>(logits,n,di,dv);
    if(!cuda_ok(cudaGetLastError(),"argmax")||
       !cuda_ok(cudaMemcpy(index_host,di,sizeof(int),cudaMemcpyDeviceToHost),
                "argmax index")) return 0;
    if(value_host&&!cuda_ok(cudaMemcpy(value_host,dv,sizeof(float),cudaMemcpyDeviceToHost),
                            "argmax value")) return 0;
    return 1;
}
extern "C" int coli_cuda_expert_group_resident_reserve(int device,int D,int I){
    DeviceContext *ctx=find_ctx(device); if(D<1||I<1||!select_ctx(ctx)) return 0;
    size_t xb=(size_t)64*D*sizeof(float),ib=(size_t)64*I*sizeof(float);
    return reserve(&ctx->x,&ctx->x_cap,xb)&&reserve(&ctx->y,&ctx->y_cap,xb)&&
           reserve(&ctx->gate,&ctx->gate_cap,ib)&&reserve(&ctx->up,&ctx->up_cap,ib)&&
           reserve(&ctx->ac,&ctx->ac_cap,(size_t)(D+64)*sizeof(float))&&
           reserve_bytes(&ctx->group_desc,&ctx->group_desc_cap,(size_t)64*sizeof(GroupDesc));
}

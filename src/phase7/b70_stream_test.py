...
covered by src/phase7/cpu_stream_test.py, which exercises the same
ExpertStreamer against dequantize_to_dtype; end-to-end agreement is covered
by src/phase7/prefill_probe.py via prompt logprobs. This file covers what those
two do not: the placement bookkeeping and the dispatch decision.

Run: python src/phase7/b70_stream_test.py    (no GPU, no B70, no model)
"""

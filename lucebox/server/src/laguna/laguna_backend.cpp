...
struct LagunaHybridDecodeScratch {
    GpuResidentState gpu_state;
    StepGraph layer_graph;
    StepGraph logits_graph;
    std::vector<int32_t> selected;
    std::vector<float> weights;
    std::vector<float> mask;
    std::vector<int32_t> hot_lut;
    std::vector<float> hot_valid;
    bool single_graph_exact = false;

    ~LagunaHybridDecodeScratch() {
        step_graph_destroy(layer_graph);
        step_graph_destroy(logits_graph);
    }
};

static bool build_laguna_hybrid_logits_step(
        StepGraph & sg,
        const LagunaTargetWeights & w,
        ggml_backend_t backend) {
    if (sg.gf) return true;
    ...
    sg.inp_embed = ggml_new_tensor_2d(
        sg.ctx, GGML_TYPE_F32, w.n_embd, 1);
    ggml_set_input(sg.inp_embed);

    ggml_tensor * normed = ggml_rms_norm(sg.ctx, sg.inp_embed, 1e-6f);
    normed = ggml_mul(sg.ctx, normed, w.out_norm);
    sg.logits = ggml_mul_mat(sg.ctx, w.output, normed);
    ...
    return sg.alloc && ggml_gallocr_alloc_graph(sg.alloc, sg.gf);
}
...
bool LagunaBackend::init_hybrid_mode() {
    ...
    if (std::getenv("DFLASH_SPARK")) {
        ...
        if (const char * cs = std::getenv("DFLASH_LAGUNA_CACHE_SLOTS")) {
            cache_slots_ = std::max(0, std::atoi(cs));
        }
        // Reactive cache fills copy one expert's three quantized matrices from
        // ordinary host memory before dispatch. That wins for the small
        // experts Spark was designed around, but large-expert models turn the
        // cache into a pageable H2D copy tax. Laguna S 2.1 Q4 uses ~5.3 MiB per
        // expert: measured at a 27 GiB target, the auto five-slot cache reduced
        // steady decode from 24.7 to 11.6 tok/s. Keep explicit --spark-slots
        // authoritative, and keep SSD mode's required GPU staging cache.
        const bool cache_slots_explicit =
            std::getenv("DFLASH_LAGUNA_CACHE_SLOTS") != nullptr;
        const char * ssd_env = std::getenv("DFLASH_SPARK_SSD");
        const bool ssd_enabled =
            ssd_env && *ssd_env && std::strcmp(ssd_env, "0") != 0;
        uint64_t max_expert_bytes = 0;
        for (uint64_t bytes : layer_expert_bytes) {
            max_expert_bytes = std::max(max_expert_bytes, bytes);
        }
        static constexpr uint64_t kAutoCacheMaxExpertBytes =
            4ULL * 1024 * 1024;
        if (!cache_slots_explicit && !ssd_enabled && cache_slots_ > 0 &&
            max_expert_bytes > kAutoCacheMaxExpertBytes) {
            std::printf(
                "[spark] reactive cache auto-disabled: expert=%.2f MiB "
                "exceeds %.0f MiB copy threshold; use --spark-slots to override\n",
                max_expert_bytes / (1024.0 * 1024.0),
                kAutoCacheMaxExpertBytes / (1024.0 * 1024.0));
            cache_slots_ = 0;
        }
    ...
    const char * cold_tier = moe_hybrid_->has_ssd_store() ? "GGUF" : "RAM";
    std::printf("[laguna-hybrid] storage ready: total_hot=%d (%.2f GiB VRAM) total_cold=%d (%.2f GiB %s) source=%s\n",
                placement.total_hot,
    ...
    const dflash::common::MoeHybridLayerStorage * hot_storage = nullptr)
{
    ...
    // KV cache write. Head-major caches store all KV heads for one token in
    // a single [head_dim*n_head_kv] row; legacy caches keep
    // [head_dim, token, n_head_kv]. Match the canonical Laguna graph's layout
    // handling so the hybrid per-layer path does not reinterpret head-major
    // row strides as token/head strides.
    ggml_tensor * cache_k = cache.attn_k[(size_t)il];
    ggml_tensor * cache_v = cache.attn_v[(size_t)il];
    const bool cache_head_major =
        cache_k && cache_k->ne[0] == head_dim * n_head_kv;
    ...
    if (cache_head_major) {
        ...
        Kcur_rows = k_merge_view
            ? ggml_view_2d(sg.ctx, Kcur, n_embd_kv, n_tokens, Kcur->nb[2], 0)
            : ggml_cont_2d(sg.ctx, Kcur, n_embd_kv, n_tokens);
        Vcur_rows = v_merge_view
            ? ggml_view_2d(sg.ctx, Vcur, n_embd_kv, n_tokens, Vcur->nb[2], 0)
            : ggml_cont_2d(sg.ctx, Vcur, n_embd_kv, n_tokens);

        ggml_tensor * k_view = ggml_view_2d(sg.ctx, cache_k,
            n_embd_kv, n_tokens, cache_k->nb[1],
            cache_k->nb[1] * (size_t)kv_start);
        ggml_tensor * v_view = ggml_view_2d(sg.ctx, cache_v,
            n_embd_kv, n_tokens, cache_v->nb[1],
            cache_v->nb[1] * (size_t)kv_start);
        ggml_build_forward_expand(sg.gf, ggml_cpy(sg.ctx, Kcur_rows, k_view));
        ggml_build_forward_expand(sg.gf, ggml_cpy(sg.ctx, Vcur_rows, v_view));
    } else {
        Kcur_rows = ggml_permute(sg.ctx, Kcur, 0, 2, 1, 3);
        Vcur_rows = ggml_permute(sg.ctx, Vcur, 0, 2, 1, 3);
    ...
    if (cache_head_major) {
        ggml_tensor * Kview = ggml_view_3d(sg.ctx, cache_k,
            head_dim, n_head_kv, kv_len,
            ggml_row_size(cache_k->type, head_dim), cache_k->nb[1], 0);
        ggml_tensor * Vview = ggml_view_3d(sg.ctx, cache_v,
            head_dim, n_head_kv, kv_len,
            ggml_row_size(cache_v->type, head_dim), cache_v->nb[1], 0);
        Kfa = ggml_permute(sg.ctx, Kview, 0, 2, 1, 3);
        Vfa = ggml_permute(sg.ctx, Vview, 0, 2, 1, 3);
    } else {
        Kfa = ggml_view_3d(sg.ctx, cache_k,
            head_dim, kv_len, n_head_kv,
            cache_k->nb[1], cache_k->nb[2], 0);
        Vfa = ggml_view_3d(sg.ctx, cache_v,
            head_dim, kv_len, n_head_kv,
...
bool LagunaBackend::hybrid_forward_one_token(
        int32_t tok, int kv_pos,
        std::vector<float> & act_cur,
        int32_t & argmax_out,
        LagunaHybridDecodeScratch & scratch) {
    const int hidden = w_.n_embd;
    ...
    // The single graph masks every route absent from its GPU residency LUT.
    // It is therefore exact only when storage has no cold experts at all.
    // RAM-cold execution must stop at each router and join MoeExpertCompute's
    // CPU contribution before the next layer consumes the residual.
    const bool g_single_graph =
        scratch.single_graph_exact &&
        std::getenv("DFLASH_LAGUNA_NO_SINGLE_GRAPH") == nullptr &&
        !moe_hybrid_->has_ssd_store();
    if (g_single_graph) {
        ...
        if (!laguna_step_hybrid(backend_, w_, cache_, act_cur.data(), 1, kv_pos, _nm,
                                *moe_hybrid_, _sg_logits, &_sg_sel,
                                kvflash_active() ? &kvflash_pager_ : nullptr))
            return false;
        // All routed experts are guaranteed resident in this branch. Preserve
        // routing observations, but do not schedule reactive cache uploads.
    ...
    // Reuse GPU state, graph allocation, and host staging buffers for the
    // whole request. Graph metadata still changes with kv_pos, but the large
    // gallocr buffer is reserved once instead of once per generated token.
    GpuResidentState & gpu_state = scratch.gpu_state;
    StepGraph & layer_sg = scratch.layer_graph;
    std::vector<int32_t> & selected = scratch.selected;
    std::vector<float> & weights_buf = scratch.weights;
    selected.resize((size_t)w_.n_expert_used);
    weights_buf.resize((size_t)w_.n_expert_used);

    // The exact per-layer fallback writes KV at literal view offsets (no
    // set_rows), which a kvflash pool cannot express once chunks relocate.
    if (kvflash_active()) {
        std::fprintf(stderr, "[kvflash] exact Laguna RAM-cold decode is not "
                             "pool-aware; disable kvflash for hybrid offload\n");
        return false;
    }

    if (!gpu_state.valid() &&
        !init_gpu_resident_state(gpu_state, backend_, hidden)) return false;
    ...
    for (int il = 0; il < w_.n_layer; ++il) {
        ...
        if (!build_laguna_layer_prefn_step(
                layer_sg, w_, cache_, backend_, il, kv_pos, 1,
        ...
        // A decode row is causal by construction. Full-attention layers see
        // the whole prefix; SWA layers must still mask keys older than the
        // configured sliding window.
        if (layer_sg.attn_mask) {
            const int kv_len = kv_pos + 1;
            scratch.mask.assign((size_t)kv_len, 0.0f);
            if (!is_full) {
                const int win_lo =
                    std::max(0, kv_pos - w_.sliding_window + 1);
                std::fill(scratch.mask.begin(),
                          scratch.mask.begin() + win_lo, -INFINITY);
            }
            ggml_backend_tensor_set(layer_sg.attn_mask, scratch.mask.data(), 0,
                                    sizeof(float) * (size_t)kv_len);
        }

        static const bool g_fuse_dec_requested =
            std::getenv("DFLASH_LAGUNA_FUSE_FFN") != nullptr;
        auto & layer_storage = moe_hybrid_->layers[(size_t)il];
        const bool g_fuse_dec =
            g_fuse_dec_requested && !moe_hybrid_->has_ssd_store() &&
            layer_storage.cold_expert_ids.empty();
        if (g_fuse_dec && !is_dense && layer_sg.hot_local_lut) {
            scratch.hot_lut.resize((size_t)w_.n_expert);
            scratch.hot_valid.resize((size_t)w_.n_expert);
            for (int g = 0; g < w_.n_expert; ++g) {
                const int loc = g < (int)layer_storage.hot_local_by_global.size()
                    ? layer_storage.hot_local_by_global[(size_t)g] : -1;
                scratch.hot_lut[(size_t)g] = loc >= 0 ? loc : 0;
                scratch.hot_valid[(size_t)g] = loc >= 0 ? 1.0f : 0.0f;
            }
            ggml_backend_tensor_set(
                layer_sg.hot_local_lut, scratch.hot_lut.data(), 0,
                sizeof(int32_t) * (size_t)w_.n_expert);
            ggml_backend_tensor_set(
                layer_sg.valid_lut, scratch.hot_valid.data(), 0,
        ...
        if (is_dense) {
            ...
            // This branch is enabled only when the layer has no cold experts,
            // so every selected contribution is present in the fused graph.
            ggml_backend_tensor_copy(
                layer_sg.hidden_input, gpu_state.act_cur);
            ggml_backend_tensor_get(
                layer_sg.moe_selected[0], selected.data(), 0,
                sizeof(int32_t) * selected.size());
            if (routing_stats_) {
                routing_stats_->observe(
                    il, selected.data(), (int)selected.size());
            }
        } else {
            ...
            if (moe_hybrid_->has_ssd_store()) {
                std::string ssd_error;
                if (!dflash::common::moe_hybrid_ssd_cache_selected(
                        *moe_hybrid_, il, selected.data(),
                        (int)selected.size(), backend_, &ssd_error)) {
                    return false;
                }
            }
    ...
    // Keep the final hidden state on device. The request-persistent logits
    // graph consumes it with D2D copy and returns only one i32 argmax, avoiding
    // a hidden D2H/H2D round trip, a full-vocabulary D2H transfer, and per-token
    // graph/context/gallocr allocation.
    const auto _t_logits = _prof ? _pnow() : _pclk::time_point{};
    if (!build_laguna_hybrid_logits_step(
            scratch.logits_graph, w_, backend_)) {
        return false;
    }
    ggml_backend_tensor_copy(
        gpu_state.act_cur, scratch.logits_graph.inp_embed);
    if (ggml_backend_graph_compute(
            backend_, scratch.logits_graph.gf) != GGML_STATUS_SUCCESS) {
        return false;
    }
    ggml_backend_tensor_get(
        scratch.logits_graph.argmax_tokens, &argmax_out, 0,
        sizeof(argmax_out));
    if (std::getenv("DFLASH_LAGUNA_LOGIT_MARGIN")) {
        static uint64_t margin_call = 0;
        const int n_logits = (int)scratch.logits_graph.logits->ne[0];
        std::vector<float> logits((size_t)n_logits);
        ggml_backend_tensor_get(
            scratch.logits_graph.logits, logits.data(), 0,
            sizeof(float) * logits.size());
        int second = -1;
        float second_value = -INFINITY;
        for (int i = 0; i < n_logits; ++i) {
            if (i != argmax_out && logits[(size_t)i] > second_value) {
                second = i;
                second_value = logits[(size_t)i];
            }
        }
        std::fprintf(stderr,
                     "[lag-logit-margin] call=%llu best=%d value=%.9g "
                     "second=%d value=%.9g margin=%.9g\n",
                     (unsigned long long)++margin_call, argmax_out,
                     logits[(size_t)argmax_out], second, second_value,
                     logits[(size_t)argmax_out] - second_value);
    }
    if (_prof) {
...
GenerateResult LagunaBackend::generate_hybrid(const GenerateRequest & req,
    ...
    if (!moe_hybrid_->has_ssd_store() && !ensure_moe_expert_compute()) {
    ...
    auto t_pf0 = std::chrono::steady_clock::now();
    const int prefill_chunk = moe_hybrid_->has_ssd_store()
        ? 1
        : std::min(args_.chunk, N);
    ...
    std::vector<float> chunk_residuals;
    std::vector<float> chunk_post;
    std::vector<int32_t> chunk_selected;
    std::vector<float> chunk_weights;
    std::vector<float> ffn_batch_out;
    std::vector<int32_t> selected_cold;
    ...
    for (int il = 0; il < w_.n_layer; ++il) {
        ...
        for (int chunk_start = 0; chunk_start < N; chunk_start += prefill_chunk) {
            ...
            // Set positions
            pos_data.resize((size_t)chunk_len);
            ...
            if (is_dense) {
                ...
                layer_out.resize((size_t)chunk_len * (size_t)hidden);
                ...
                chunk_residuals.resize(
                    (size_t)chunk_len * (size_t)hidden);
                chunk_post.resize(
                    (size_t)chunk_len * (size_t)hidden);
                chunk_selected.resize(
                    (size_t)chunk_len * (size_t)n_expert_used);
                chunk_weights.resize(
                    (size_t)chunk_len * (size_t)n_expert_used);
                ...
                if (moe_hybrid_->has_ssd_store()) {
                    if (!dflash::common::moe_hybrid_ssd_cache_selected(
                            *moe_hybrid_, il, chunk_selected.data(),
                            chunk_len * n_expert_used, backend_, &ffn_error)) {
                        std::fprintf(stderr, "[laguna-ssd] layer %d: %s\n",
                                     il, ffn_error.c_str());
                        result.fail(GenerateErrorCode::BackendSpecific,
                                    "ssd_expert_cache: " + ffn_error);
                        step_graph_destroy(prefill_sg);
                        if (ffn_hot_alloc) ggml_gallocr_free(ffn_hot_alloc);
                        if (ffn_cold_alloc) ggml_gallocr_free(ffn_cold_alloc);
                        return result;
                ...
                if (storage.cold_expert_ids.empty()) {
                    ...
                    ffn_ok = eval_moe_hot_only_batched(
                            ...
                            chunk_len, ffn_batch_out, &ffn_error,
                            &ffn_hot_alloc);
                } else if (!moe_hybrid_->has_ssd_store() &&
                           moe_hybrid_->has_mmap() &&
                           !moe_hybrid_->layer_regions.empty() &&
                           stream_engine_.is_ready() && chunk_len >= 16 &&
                           !storage.cold_expert_ids.empty()) {
                    // Advise only cold experts routed by this chunk. Advising
                    // the layer's entire cold set scans thousands of unrelated
                    // mappings before every batched RAM/CPU evaluation.
                    selected_cold.clear();
                    for (int32_t global_id : chunk_selected) {
                        if (global_id < 0 ||
                            global_id >=
                                (int)storage.cold_local_by_global.size()) {
                            continue;
                        }
                        const bool resident =
                            global_id <
                                (int)storage.hot_local_by_global.size() &&
                            storage.hot_local_by_global[
                                (size_t)global_id] >= 0;
                        if (!resident &&
                            storage.cold_local_by_global[
                                (size_t)global_id] >= 0) {
                            selected_cold.push_back(global_id);
                        }
                    }
                    std::sort(selected_cold.begin(), selected_cold.end());
                    selected_cold.erase(
                        std::unique(
                            selected_cold.begin(), selected_cold.end()),
                        selected_cold.end());
                    const auto & regions =
                        moe_hybrid_->layer_regions[(size_t)il];
                    stream_engine_.prefetch_cold_experts(
                        moe_hybrid_->mmap_data, moe_hybrid_->mmap_size,
                        regions, selected_cold.data(),
                        (int)selected_cold.size());
                    ffn_ok = eval_moe_hybrid_ffn_batched(
                            backend_, cpu_be, chunk_cfg, chunk_desc, storage,
                            chunk_post.data(),
                            chunk_selected.data(),
    ...
    LagunaHybridDecodeScratch decode_scratch;
    decode_scratch.single_graph_exact = true;
    for (int il = w_.n_layer_dense_lead; il < w_.n_layer; ++il) {
        if (!moe_hybrid_->layers[(size_t)il].cold_expert_ids.empty()) {
            decode_scratch.single_graph_exact = false;
            break;
        }
    }
    auto t_g0 = std::chrono::steady_clock::now();
...
bool LagunaBackend::build_hybrid_storage_from_file(
    ...
    // Transfer mmap ownership to hybrid storage. The SSD tier duplicates the
    // source descriptor before this function closes it.
    ...
    const char * ssd_raw = std::getenv("DFLASH_SPARK_SSD");
    const bool ssd_enabled = ssd_raw && *ssd_raw && std::strcmp(ssd_raw, "0") != 0;
    if (ssd_enabled) hybrid_cfg.materialize_cold_experts = false;
    std::vector<MoeLayerDesc> layer_descs((size_t)w_.n_layer);
    for (int il = 0; il < w_.n_layer; ++il) {
        layer_descs[(size_t)il] = make_moe_layer_desc(w_.layers[(size_t)il]);
    }
    int cache_slots = 0;
    if (const char * cs = std::getenv("DFLASH_LAGUNA_CACHE_SLOTS")) {
        cache_slots = std::max(0, std::atoi(cs));
    } else if (cache_slots_ >= 0) {
        cache_slots = cache_slots_;
    }
    if (ssd_enabled) cache_slots = std::max(cache_slots, w_.n_expert_used);
    const bool ok = build_moe_hybrid_storage_from_file_with_mmap(
        hybrid_cfg, backend_, placement, layer_descs, layer_file_data,
        mmap_addr, file_size, _region.fd, *hybrid, &err, cache_slots);
#if !defined(_WIN32)

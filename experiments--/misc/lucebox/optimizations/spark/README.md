...
For models that also exceed practical system-RAM capacity, add
`--spark-ssd`. The cold tier then stays file-backed in the GGUF and cache
misses pass through a bounded pinned-host staging pool before synchronous,
completion-safe publication in GPU expert slots:

```bash
dflash_server model.gguf --spark --spark-ssd --spark-vram 28 --spark-slots 16
```

This bounds host expert allocation, but performance depends on the learned
placement, cache size, storage, and operating-system page cache. Shutdown logs
report cumulative `[spark-ssd]` hits, misses, evictions, bytes read, read rate,
and upload time.

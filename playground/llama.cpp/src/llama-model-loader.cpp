...
// implemented in ggml-cpu: (primary base -> replica base) registry for the
// least-loaded expert assignment in the fused MoE kernel
extern "C" void ggml_fused_moe_register_replica(const void * primary, void * replica);
extern "C" int  ggml_fused_moe_copy_node(int home, int c, int n_nodes, int n_copies);
extern "C" bool ggml_backend_cuda_register_host_buffer(void * buffer, size_t size);
#endif
...
bool llama_model_loader::load_all_data(
...
#if defined(__linux__)
    // GGML_EXPERT_SHARD=1: bind each expert block's pages to its home NUMA node BEFORE
    // any data is written, so the loader's first-touch writes place pages directly on
    // the home node and the compute-side migration pass becomes a no-op. Only effective
    // with --no-mmap (mmap-loaded tensors are file-backed and not first-touch placed).
    // Home assignment must match ggml-cpu's ggml_fused_moe_expert_home(): contiguous
    // blocks, expert e -> node (e * n_nodes / n_as).
    {
        const char * shard_env = getenv("GGML_EXPERT_SHARD");
        if (shard_env && atoi(shard_env) != 0 && !use_mmap) {
            int n_nodes = 0;
            char npath[64];
            while (n_nodes < 64) {
                snprintf(npath, sizeof(npath), "/sys/devices/system/node/node%d", n_nodes);
                if (access(npath, F_OK) != 0) {
                    break;
                }
                n_nodes++;
            }
            if (n_nodes > 1 && n_nodes <= 8) {
                const long page = sysconf(_SC_PAGESIZE);
                int n_bound = 0;
                for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
                    if (t->buffer == nullptr || t->data == nullptr || !ggml_backend_buffer_is_host(t->buffer)) {
                        continue;
                    }
                    if (t->ne[2] < n_nodes || strstr(t->name, "_exps") == nullptr) {
                        continue;
                    }
                    const int64_t n_as = t->ne[2];
                    const size_t  per  = t->nb[2];
                    for (int g = 0; g < n_nodes; g++) {
                        const int64_t e0 = (int64_t)  g      * n_as / n_nodes;
                        const int64_t e1 = (int64_t) (g + 1) * n_as / n_nodes;
                        char * s0 = (char *) t->data + e0*per;
                        char * s1 = (char *) t->data + e1*per;
                        s0 = (char *) (((uintptr_t) s0 + page - 1) & ~(uintptr_t)(page - 1));
                        s1 = (char *) ( (uintptr_t) s1            & ~(uintptr_t)(page - 1));
                        if (s1 <= s0) {
                            continue;
                        }
                        unsigned long mask = 1UL << g;
                        // MPOL_BIND = 2, no move flags: tags the VMA only, pages fault home
                        syscall(SYS_mbind, s0, (size_t)(s1 - s0), 2, &mask, sizeof(mask)*8 + 1, 0);
                    }
                    n_bound++;
                }
                if (n_bound > 0) {
                    fprintf(stderr, "expert-shard: pre-bound %d expert tensors across %d NUMA nodes (first-touch placement)\n",
                        n_bound, n_nodes);
                }
            }
        }
    ...
    std::set<ggml_tensor *> fastload_done;
#if defined(__linux__)
    // GGML_EXPERT_SHARD parallel fast-load. The stock path pays ~36M single-threaded
    // page faults for a 145 GB model (~1 GB/s wall). Phase A populates destination
    // pages N-wide (policies from the pre-bind above decide placement, MADV_HUGEPAGE
    // first cuts fault count 512x); phase B fills tensors N-wide with pread().
    // Thread count: GGML_FASTLOAD_THREADS, default 16 (measured optimum: more threads
    // evict the page cache faster than they fill pages when the cache is cold).
    {
        const char * env = getenv("GGML_EXPERT_SHARD");
        if (env && atoi(env) != 0 && !use_mmap && !check_tensors) {
            struct fl_slice { int fd; size_t off; uint8_t * dst; size_t len; };
            std::vector<fl_slice> slices;
            std::set<ggml_backend_buffer_t> bufs_seen;
            std::vector<std::pair<uint8_t *, size_t>> ranges;
            for (ggml_tensor * t = ggml_get_first_tensor(ctx); t; t = ggml_get_next_tensor(ctx, t)) {
                const auto * w = get_weight(ggml_get_name(t));
                if (!w || !t->buffer || !t->data || !ggml_backend_buffer_is_host(t->buffer)) {
                    continue;
                }
                const int fd = files.at(w->idx)->file_id();
                const size_t n = ggml_nbytes(t);
                const size_t CH = 8u << 20;
                for (size_t o = 0; o < n; o += CH) {
                    slices.push_back({fd, w->offs + o, (uint8_t *) t->data + o, std::min(CH, n - o)});
                }
                fastload_done.insert(t);
                if (bufs_seen.insert(t->buffer).second) {
                    ranges.emplace_back((uint8_t *) ggml_backend_buffer_get_base(t->buffer),
                                        ggml_backend_buffer_get_size(t->buffer));
                }
            }

            // GGML_EXPERT_REPLICAS=k: allocate k extra copies of every expert tensor,
            // copy c bound to node ggml_fused_moe_copy_node(home, c) (k=1: antipodal
            // partner, the recommended setting; k=n_nodes-1: full replication, measured
            // null beyond k=1), filled by the same parallel pread pass below and
            // registered with ggml-cpu for least-loaded assignment
            uint8_t * rep_base_ptr = nullptr; // replica range: never shipped to the GPU, skipped by expert-pin
            const char * rep_env = getenv("GGML_EXPERT_REPLICAS");
            if (rep_env && atoi(rep_env) > 0 && !slices.empty()) {
                int n_nodes = 0;
                char npath2[64];
                while (n_nodes < 64) {
                    snprintf(npath2, sizeof(npath2), "/sys/devices/system/node/node%d", n_nodes);
                    if (access(npath2, F_OK) != 0) break;
                    n_nodes++;
                }
                std::vector<ggml_tensor *> reps;
                size_t rep_total = 0;
                for (ggml_tensor * t = ggml_get_first_tensor(ctx); t; t = ggml_get_next_tensor(ctx, t)) {
                    const auto * w = get_weight(ggml_get_name(t));
                    if (!w || !t->buffer || !t->data || !ggml_backend_buffer_is_host(t->buffer)) continue;
                    if (t->ne[2] < n_nodes || strstr(t->name, "_exps") == nullptr) continue;
                    reps.push_back(t);
                    rep_total += GGML_PAD(ggml_nbytes(t), 4096);
                }
                const int k = std::min(atoi(rep_env), n_nodes - 1);
                if (n_nodes > 1 && k > 0 && !reps.empty()) {
                    const size_t rep_total_all = rep_total * k;
                    uint8_t * rep_base = (uint8_t *) mmap(NULL, rep_total_all, PROT_READ | PROT_WRITE,
                                                          MAP_PRIVATE | MAP_ANONYMOUS | MAP_NORESERVE, -1, 0);
                    if (rep_base != MAP_FAILED) {
                        rep_base_ptr = rep_base;
                        const long page2 = sysconf(_SC_PAGESIZE);
                        madvise(rep_base, rep_total_all, MADV_HUGEPAGE);
                        size_t off = 0;
                        for (int c = 1; c <= k; c++) {
                            for (ggml_tensor * t : reps) {
                                const auto * w = get_weight(ggml_get_name(t));
                                const int64_t n_as = t->ne[2];
                                const size_t  per  = t->nb[2];
                                const size_t  n    = ggml_nbytes(t);
                                uint8_t * dst = rep_base + off;
                                for (int home = 0; home < n_nodes; home++) {
                                    const int     part = ggml_fused_moe_copy_node(home, c, n_nodes, k);
                                    const int64_t e0 = (int64_t)  home      * n_as / n_nodes;
                                    const int64_t e1 = (int64_t) (home + 1) * n_as / n_nodes;
                                    char * s0 = (char *) dst + e0*per;
                                    char * s1 = (char *) dst + e1*per;
                                    s0 = (char *) (((uintptr_t) s0 + page2 - 1) & ~(uintptr_t)(page2 - 1));
                                    s1 = (char *) ( (uintptr_t) s1              & ~(uintptr_t)(page2 - 1));
                                    if (s1 <= s0) continue;
                                    unsigned long mask = 1UL << part;
                                    syscall(SYS_mbind, s0, (size_t)(s1 - s0), 2, &mask, sizeof(mask)*8 + 1, 0);
                                }
                                const int fd = files.at(w->idx)->file_id();
                                const size_t CH = 8u << 20;
                                for (size_t o = 0; o < n; o += CH) {
                                    slices.push_back({fd, w->offs + o, dst + o, std::min(CH, n - o)});
                                }
                                ggml_fused_moe_register_replica(t->data, dst);
                                off += GGML_PAD(n, 4096);
                            }
                        }
                        ranges.emplace_back(rep_base, rep_total_all);
                        fprintf(stderr, "expert-replicas: %.1f GB in %d extra copies bound across nodes (%zu tensors)\n",
                            rep_total_all / 1e9, k, reps.size());
                    }
                }
            }

            if (!slices.empty()) {
                const char * nt_env = getenv("GGML_FASTLOAD_THREADS");
                const int NT = nt_env && atoi(nt_env) > 0 ? std::min(atoi(nt_env), 192) : 16;
                const int64_t tp0 = ggml_time_us();
#ifndef MADV_POPULATE_WRITE
#define MADV_POPULATE_WRITE 23
#endif
                {
                    std::vector<std::pair<uint8_t *, size_t>> pop;
                    const size_t PC = 512u << 20;
                    for (auto & r : ranges) {
                        madvise(r.first, r.second, MADV_HUGEPAGE);
                        for (size_t o = 0; o < r.second; o += PC) {
                            pop.emplace_back(r.first + o, std::min(PC, r.second - o));
                        }
                    }
                    std::atomic<size_t> ri{0};
                    std::vector<std::thread> th;
                    for (int i = 0; i < NT; i++) {
                        th.emplace_back([&] {
                            size_t k;
                            while ((k = ri.fetch_add(1)) < pop.size()) {
                                madvise(pop[k].first, pop[k].second, MADV_POPULATE_WRITE);
                            }
                        });
                    }
                ...
                size_t total = 0;
                for (auto & s : slices) { total += s.len; }
                std::atomic<size_t> si{0};
                std::atomic<bool> io_ok{true};
                std::vector<std::thread> th;
                for (int i = 0; i < NT; i++) {
                    th.emplace_back([&] {
                        size_t k;
                        while ((k = si.fetch_add(1)) < slices.size() && io_ok) {
                            auto & s = slices[k];
                            size_t done = 0;
                            while (done < s.len) {
                                ssize_t r = pread(s.fd, s.dst + done, s.len - done, s.off + done);
                                if (r <= 0) { io_ok = false; return; }
                                done += (size_t) r;
                            }
                        }
                    });
                ...
                if (!io_ok) {
                    fastload_done.clear(); // serial path below reloads everything
                    fprintf(stderr, "fastload: pread failed, falling back to serial load\n");
                } else {
                    fprintf(stderr, "fastload: %.1f GB - populate %.1fs, fill %.1fs (%.1f GB/s, %d threads)\n",
                        total / 1e9, (tp1 - tp0) / 1e6, (tp2 - tp1) / 1e6,
                        total / 1e9 / std::max((tp2 - tp1) / 1e6, 1e-3), NT);
                }

                // GGML_EXPERT_PIN=1: page-lock the primary expert ranges AFTER NUMA
                // placement (mmap + mbind + populate, THEN cudaHostRegister - order
                // matters: cudaMallocHost pins before policy and wrecks residency).
                // lets the prefill expert stream run at DMA rate instead of pageable
                // staging (measured 5.5 GB/s staging vs ~14 GB/s wire at Gen3 x16).
                // replicas are skipped: they never ship to the GPU. requires
                // GGML_CUDA_REGISTER_HOST=1 (the ggml wrapper no-ops without it).
                const char * pin_env = getenv("GGML_EXPERT_PIN");
                if (pin_env && atoi(pin_env) != 0 && io_ok) {
                    const int64_t tpin0 = ggml_time_us();
                    size_t pinned = 0;
                    for (auto & r : ranges) {
                        if (rep_base_ptr && r.first == rep_base_ptr) {
                            continue;
                        }
                        if (ggml_backend_cuda_register_host_buffer(r.first, r.second)) {
                            pinned += r.second;
                        } else {
                            fprintf(stderr, "expert-pin: register FAILED for %.1f GB range\n", r.second / 1e9);
                        }
                    }
                    fprintf(stderr, "expert-pin: %.1f GB page-locked for DMA in %.1fs\n",
                        pinned / 1e9, (ggml_time_us() - tpin0) / 1e6);
                }
            }
        }
    }
